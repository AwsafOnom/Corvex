...
2. **Measure X on each detected object**, and accept the per-object
   ...
   (in `corvex.skills._shared.fourier_reflection`) can do the
   per-crop detection.
...
2. **SAM instance segmentation (irregular / arbitrary shapes, or when
   ...
   `corvex.skills._shared.sam`. SAM detects individual instances
...
```
from corvex.skills._shared.sam import run_sam_analysis
