...
# home (default ``~/.corvex/graduated_skills``) so they survive a
# ``pip`` upgrade — they live outside the installed package tree.
# ``CORVEX_HOME`` relocates the whole ``.corvex`` store. This dir is
...
def corvex_home() -> Path:
    """Return the persistent Corvex home dir (``~/.corvex`` by default).

    Honors the ``CORVEX_HOME`` env var. Re-read on each call so the
    location can change mid-process (mainly for tests)."""
    return Path(os.environ.get("CORVEX_HOME") or (Path.home() / ".corvex")).expanduser()
...
def graduated_skills_dir() -> Path:
    ...
    return corvex_home() / "graduated_skills"
...
# (``corvex memory`` / UI panel) still work so a user can inspect/manage it.
#
# Resolution order: the ``CORVEX_MEMORY`` env var overrides (truthy → on,
# falsy → off); otherwise the persisted setting in ``corvex_home()/config.json``;
...
def _config_path() -> Path:
    return corvex_home() / "config.json"
...
def memory_enabled() -> bool:
    ...
    env = os.environ.get("CORVEX_MEMORY", "").strip().lower()
    if env in _TRUTHY:
        return True
    if env in _FALSY:
        return False
...
# Users can add their own skill bundles without modifying Corvex source
# by pointing the ``CORVEX_SKILLS_PATH`` env var at one (or several,
...
def _skill_roots() -> List[Path]:
    ...
    Precedence (highest first): user roots from ``$CORVEX_SKILLS_PATH``,
    then the persistent graduated-skills store (``~/.corvex/...``), then
    the built-in ``corvex/skills/`` tree. So an upgraded/graduated skill
    ...
    also lists the home dir in ``$CORVEX_SKILLS_PATH`` doesn't get a
    double-counted root.
    """
    candidates: List[Path] = []
    extra = os.environ.get("CORVEX_SKILLS_PATH", "").strip()
    if extra:
        for raw in extra.split(os.pathsep):
            ...
            if p.is_dir():
                ...
                _logger.warning(
                    "CORVEX_SKILLS_PATH entry not found or not a directory: %s", p
                )
...
def list_skills(domain: str = "curve_fitting") -> list:
    ...
    User-provided skill roots (via ``$CORVEX_SKILLS_PATH``) and the
...
def list_all_skills() -> dict:
    ...
    ``$CORVEX_SKILLS_PATH`` and the built-in ``corvex/skills/``) for
...
def load_skill(skill: str, domain: str = "curve_fitting") -> Dict:
    ...
    Args:
        skill: Either a built-in skill name (e.g. "xps") which resolves to
            ``corvex/skills/{domain}/{name}.md``, or a path to a custom
            ``.md`` file.
        domain: Skill domain subdirectory (default: "curve_fitting").
...
def _resolve_skill_path(skill: str, domain: str) -> Path:
    ...
    layout). Searches user-provided roots from ``$CORVEX_SKILLS_PATH``
    first (so user bundles can override built-in ones of the same
    name), then the built-in ``corvex/skills/`` tree.
