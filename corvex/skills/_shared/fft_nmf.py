...
TOOL_SPEC = ToolSpec(
    ...
    import_line="from corvex.skills._shared.fft_nmf import run_fft_nmf_analysis",
    signature="run_fft_nmf_analysis(image_array, params=None) -> dict",
    agents=["image_analysis"],
    when_to_use=(
        "The EXPLORATORY / unsupervised baseline for spatially varying structure "
        "when the heterogeneity is UNKNOWN and you want to discover whatever "
        "distinct local-frequency patterns exist and where (defects, disorder, "
        "orientation/texture domains, phase-like regions) without a prior on what "
        "to look for.\n"
        "\n"
        "Do NOT default here for a SPECIFIC, nameable periodicity question — a "
        "satellite/superstructure reflection, an ordered domain or second phase "
        "with a distinct spacing, a moire, or a strain/displacement field. For "
        "those, `fourier_reflection_map` (corvex.skills._shared.fourier_reflection) "
        "is sharper and interpretable: it detects reflections with a significance, "
        "maps a chosen one (amplitude = where it lives; phase = GPA strain), and "
        "null-gates the result. Reach for FFT-NMF when the question is 'what "
        "domains exist?'; reach for fourier_reflection_map when you can name or "
        "first detect the reflection of interest. (FFT-NMF remains a fine fallback "
        "for orientation-only domains such as twins, where reflections share |k| "
        ...
        "thresholding or clustering abundances to localize distinct regions), it is "
        "a self-contained exploratory pipeline.\n"
        "\n"
