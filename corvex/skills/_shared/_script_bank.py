...
Records live at ``corvex_home()/script_bank/<domain>/<id>.json`` — a sibling
of ``graduated_skills/`` and ``distill_staging/``, honoring ``$CORVEX_HOME``.
...
from ..loader import corvex_home, memory_enabled, _TRUTHY, _FALSY
from ._graduation import safe_path_component, warn_if_ephemeral_store


def bank_dir() -> Path:
    """Root of the script bank (honors ``$CORVEX_HOME``)."""
    return corvex_home() / "script_bank"


def bank_enabled() -> bool:
    """Whether the bank write hooks are active.

    ``CORVEX_SCRIPT_BANK`` overrides in both directions (so the bank can run
    ...
    """
    flag = os.environ.get("CORVEX_SCRIPT_BANK", "").strip().lower()
    if flag in _FALSY:
        return False
    if flag in _TRUTHY:
        return True
...
#: signal. Overridable via $CORVEX_BANK_PROVEN_N.
_DEFAULT_PROVEN_N = 3


def proven_n() -> int:
    raw = os.environ.get("CORVEX_BANK_PROVEN_N", "").strip()
    if raw:
        try:
