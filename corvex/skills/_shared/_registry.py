...
Walks ``corvex/skills/_shared/`` and every per-skill bundle once, collects
``TOOL_SPEC`` (single) or ``TOOL_SPECS`` (list) attributes from each public
module, filters by agent tag, and caches the result.

Adding a new tool:
    1. Write the function as a module either under ``corvex/skills/_shared/``
       (cross-skill helper) or inside the relevant ``corvex/skills/<domain>/<name>/``
...
def _shared_specs() -> tuple[ToolSpec, ...]:
    """Specs declared by modules under ``corvex/skills/_shared/``.
    ...
    of the other tools.
    """
    from . import __path__ as _shared_path  # corvex/skills/_shared/

    collected: list[ToolSpec] = []
    for info in pkgutil.iter_modules(_shared_path):
        if info.name.startswith("_"):
            continue
        module_path = f"corvex.skills._shared.{info.name}"
        try:
...
def _per_skill_specs() -> dict[tuple[str, str], tuple[ToolSpec, ...]]:
    ...
    """
    from corvex.skills.loader import list_all_skills, _SKILLS_DIR
        ...
        for name in names:
            ...
            for py_file in sorted(skill_dir.glob("*.py")):
                if py_file.stem.startswith("_"):
                    continue
                module_path = f"corvex.skills.{domain}.{name}.{py_file.stem}"
                try:
                    mod = importlib.import_module(module_path)
                except Exception as exc:
                    _logger.debug("Skipping %s: %s", module_path, exc)
