...
Staging lives at ``corvex_home()/distill_staging/<domain>/<id>.json`` — a sibling
...
from ..loader import corvex_home, load_skill, list_skills, graduated_skills_dir
...
def staging_dir() -> Path:
    """Root of the raw-solution staging buffer (honors ``$CORVEX_HOME``)."""
    return corvex_home() / "distill_staging"
...
def consolidate_min_n() -> int:
    """Readiness threshold for new-skill consolidation (``$CORVEX_CONSOLIDATE_N``)."""
    import os
    raw = os.environ.get("CORVEX_CONSOLIDATE_N", "").strip()
    if raw:
        try:
