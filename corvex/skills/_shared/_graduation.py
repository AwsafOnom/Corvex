...
under ``corvex/skills/_shared/`` (not inside ``agents/sim_agents/``) so
the analysis, planning, meta, and CLI code paths can import it WITHOUT
pulling in ``ase`` (an optional dependency that ``corvex.agents.sim_agents``
hard-imports). The module itself is pure stdlib + lazy ``yaml``/loader —
no heavy imports. ``corvex/agents/sim_agents/skill_graduation.py`` is now
...
Default storage: ~/.corvex/graduated_skills/<domain>/<name>/<name>.md
(see ``corvex.skills.loader.graduated_skills_dir`` — the single source
of truth, honoring ``$CORVEX_HOME``).
"""
...
def warn_if_ephemeral_store() -> None:
    ...
    Corvex is commonly run in Docker; inside a container the default home
    (``~/.corvex``) lives on the ephemeral container filesystem, so graduated/
    ...
    a conservative heuristic: in a container AND ``$CORVEX_HOME`` is unset →
    advise mounting a volume. Advisory only; never blocks or relocates.
    """
    global _EPHEMERAL_WARNED
    if _EPHEMERAL_WARNED or os.environ.get("CORVEX_HOME"):
        return
    ...
    if in_container:
        ...
        from ..loader import corvex_home
        logging.warning(
...
# ``graduated_skills_dir()`` (honors $CORVEX_HOME dynamically); this is
...
# corvex/skills/loader.py knows about; rendered into markdown in this
    ...
    *,
    ...
    Args:
        ...
        skills_root: where graduated skills live. Defaults to the
            persistent store (~/.corvex/graduated_skills).
...
def load_graduated_skills(
    ...
    Each dict has the same shape as `corvex.skills.loader.load_skill`'s
