...
Scope/safety: caching is active ONLY when the ``CORVEX_TOOL_CACHE_DIR``
...
result or breaks a tool. Set ``CORVEX_TOOL_CACHE_DISABLE=1`` to force-disable.
"""
...
def disk_memoized(func):
    """Content-addressed disk memoization, active only under CORVEX_TOOL_CACHE_DIR.
    ...
    def wrapper(*args, **kwargs):
        cache_dir = os.environ.get("CORVEX_TOOL_CACHE_DIR")
        if not cache_dir or os.environ.get("CORVEX_TOOL_CACHE_DISABLE") == "1":
