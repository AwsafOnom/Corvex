...
corvex imports) so it can be exercised in a GSAS-II-only environment. pymatgen,
...
metadata, so this cannot be a ``corvex[gsas]`` extra.) The install compiles its
Fortran extensions against the environment's numpy. Install corvex/pymatgen
