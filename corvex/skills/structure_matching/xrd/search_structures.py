...
def register_source_preference(name: str, rank: int) -> None:
    ...
    Mirrors :func:`corvex.skills.structure_matching._backends.register_backend`
...
TOOL_SPEC = ToolSpec(
    ...
    import_line="from corvex.skills.structure_matching.xrd.search_structures import search_structures",
    ...
    parameters={
        ...
        "sources": {
            ...
            "description": (
                ...
                "environment (MP_API_KEY set → 'mp'; CORVEX_LOCAL_CIF_DIR "
                "exists → 'local')."
            ),
        },
        "output_dir": {
...
def _build_backends(
    ...
    Backends come from :mod:`corvex.skills.structure_matching._backends`,
    which carries both the built-in factories and any third-party backends
    registered via ``register_backend()`` or the
    ``corvex.structure_backends`` entry-point group.
    """
    if sources is None:
