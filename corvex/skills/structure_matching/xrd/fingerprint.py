...
Set ``CORVEX_XRD_FINGERPRINT_DB`` to the parquet path (or pass ``library_path``)
...
_ENV_DB = "CORVEX_XRD_FINGERPRINT_DB"
...
def build_fingerprint_library(
    ...
    if not PYMATGEN_AVAILABLE:
        raise RuntimeError("build_fingerprint_library requires pymatgen "
                           "(install corvex[structure-matching]).")
...
#
# Resolution order: explicit ``library_path`` -> $CORVEX_XRD_FINGERPRINT_DB ->
# the persistent per-user store (~/.corvex/xrd_fingerprints/, survives pip
...
DEFAULT_LIBRARY_URL = (
    "https://github.com/ziatdinovmax/Corvex/releases/download/"
...
def _default_store_path() -> str:
    return os.path.join(os.path.expanduser("~"), ".corvex",
...
def _load_library(library_path: Optional[str]):
    ...
    if not path:
        raise RuntimeError(
            ...
            f"one-time): run `corvex fetch-xrd-library` or call "
...
TOOL_SPEC = ToolSpec(
    ...
    import_line="from corvex.skills.structure_matching.xrd.fingerprint import search_match_pattern",
    signature=(
    ...
    parameters={
        ...
        "library_path": {"type": "str", "description": "Path to the fingerprint parquet (default: $CORVEX_XRD_FINGERPRINT_DB)."},
