...
Install dependencies: `pip install corvex[structure-matching]` (pymatgen
...
```toml
[project.entry-points."corvex.structure_backends"]
icsd = "my_package.icsd_backend:ICSDBackend"
```

Corvex discovers the entry point on import and the backend is then
addressable as `sources=["icsd"]` in `search_structures`. See
`corvex/skills/structure_matching/_backends/__init__.py` for the full
...
from corvex.skills.structure_matching.xrd.search_structures import search_structures
from corvex.skills.structure_matching.xrd.simulate_xrd import simulate_xrd_pattern
from corvex.skills.structure_matching.xrd.score_match_fast import score_xrd_match_fast
from corvex.skills.structure_matching.xrd.score_match_robust import score_xrd_match_robust
from corvex.skills.structure_matching.xrd.extract_peaks import extract_peaks
...
`corvex.skills.curve_fitting.xrd_profile.background` to estimate the
...
1. `quantify_phases_nnls` — the FIRST-choice fast quantifier. Fits the whole
   ...
   ```python
   from corvex.skills.structure_matching.xrd.quantify_nnls import quantify_phases_nnls
