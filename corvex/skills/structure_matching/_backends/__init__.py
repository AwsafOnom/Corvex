...
``_backends`` excludes it; see ``corvex/skills/loader.py:117-118``).
...
1. **Imperative** — call :func:`register_backend` from your code before
   invoking the agent::
       ...
       from corvex.skills.structure_matching._backends import register_backend
...
2. **Entry point** — declare the backend in your package's
   ``pyproject.toml``; Corvex discovers it on first import::
...
3. **Built-in** — add to :data:`_BUILTIN_BACKENDS` in this file. Reserved
   for backends shipping with Corvex itself.
...
# Built-in backends ship with Corvex. User-registered backends are added
...
def _load_entry_points() -> None:
    """Discover backends declared via the ``corvex.structure_backends`` entry-point group.

    A third-party package can register a backend without code changes
    inside Corvex by declaring it in its ``pyproject.toml``::
    ...
    except ImportError:  # Python < 3.10 (unlikely, Corvex requires 3.12+)
        return

    try:
        # Python 3.10+ accepts group= kwarg.
        eps = entry_points(group="corvex.structure_backends")
    except TypeError:
        # Older importlib.metadata: returns a dict keyed by group.
        eps = entry_points().get("corvex.structure_backends", [])
    except Exception as e:
        _logger.debug("entry_points() failed for corvex.structure_backends: %s", e)
        return
