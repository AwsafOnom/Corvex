...
from a local COD CIF archive (``CORVEX_COD_CIF_DIR``, preferred: offline, fast,
...
set CORVEX_COD_ALLOW_WEB=0 to force fully-offline (local-archive-only) operation.

Configuration (env vars):
  CORVEX_COD_DB        path to the COD SQLite metadata db (required).
  CORVEX_COD_CIF_DIR   local CIF archive root (preferred CIF source). CIFs are
                        read here by COD-hashed layout d1/d2d3/d4d5/<id>.cif,
                        flat <id>.cif, or any <id>.cif under the tree.
  CORVEX_COD_ALLOW_WEB '0'/'false' disables the crystallography.net fallback
...
class CODBackend:
    ...
    def __init__(self, db_path: Optional[str] = None, cif_dir: Optional[str] = None,
        ...
        self.db_path = db_path or os.getenv("CORVEX_COD_DB")
        self.cif_dir = cif_dir or os.getenv("CORVEX_COD_CIF_DIR")
        if allow_web is None:
            ...
            allow_web = os.getenv("CORVEX_COD_ALLOW_WEB", "1").lower() not in ("0", "false", "no")
        self.allow_web = allow_web
        self._cache_dir = Path(tempfile.gettempdir()) / "corvex_cod_cifs"
