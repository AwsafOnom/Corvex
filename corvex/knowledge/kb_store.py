...
Corvex home (``~/.corvex/knowledge_bases/<name>/`` by default;
``CORVEX_HOME`` relocates the whole store):
...
from ``corvex.knowledge`` and (lazily) ``corvex.parsers``, never from
``corvex.agents``.
"""
...
from ..skills.loader import corvex_home
...
def kb_store_dir() -> Path:
    ...
    return corvex_home() / "knowledge_bases"
...
def embedding_compat_warning(manifest: Optional[Dict[str, Any]],
    ...
    if built_with != session_embedding_model:
        return (
            ...
            f"corvex kb rebuild {manifest.get('name', '<name>')} "
...
def add_to_kb(name: str,
    ...
    if manifest.get("origin") != "built" or not (final / "sources").is_dir():
        raise ValueError(
            ...
            "only supported for KBs built with 'corvex kb create'; recreate "
    ...
    if conflicts:
        raise FileExistsError(
            ...
            "the stored sources and 'corvex kb rebuild', or recreate."
        )
