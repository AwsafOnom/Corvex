...
# by the fall-through ladder. $CORVEX_BANK_EDIT_ADAPT_SCORE raises it;
# $CORVEX_BANK_EDIT_ADAPT=0 disables the mode.
...
def try_bank_edit_adapt(host, ctx, *, domain: str, script_kind: str,
    ...
    if not exemplar:
        return None
    if (_os.environ.get("CORVEX_BANK_EDIT_ADAPT", "").strip().lower()
            in ("0", "false", "off", "no")):
        return None
    try:
        min_score = float(_os.environ.get(
            "CORVEX_BANK_EDIT_ADAPT_SCORE", BANK_EDIT_ADAPT_MIN_SCORE))
    except ValueError:
    ...
    try:
        ...
        from corvex.skills._shared._graduation import parse_json_response
