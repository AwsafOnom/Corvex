...
_BUILTIN_AGENTS = {
    0: {
        "class_path": "corvex.agents.exp_agents.curve_fitting_agent.CurveFittingAgent",
        "name": "CurveFittingAgent",
        "description": "1D data: DSC, TGA, XRD, UV-Vis, Raman, PL, IV curves, kinetics. Handles single files or series.",
        "short_name": "CurveFit",
    },
    1: {
        "class_path": "corvex.agents.exp_agents.image_analysis_agent.ImageAnalysisAgent",
    ...
    2: {
        "class_path": "corvex.agents.exp_agents.hyperspectral_analysis_agent.HyperspectralAnalysisAgent",
...
class AnalysisOrchestratorAgent:
    ...
    def _discover_plugin_agents(self) -> None:
        """Auto-register agents advertised via the 'corvex.agents' entry point group."""
        try:
            from importlib.metadata import entry_points
            eps = entry_points(group="corvex.agents")
        except Exception:
            return
    ...
    def run_task(self, task: str, context: Optional[Dict[str, Any]] = None,
        ...
        # staging buffer (~/.corvex/distill_staging).
