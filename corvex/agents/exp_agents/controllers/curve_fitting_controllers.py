...
def _resolve_parallel_workers(value: Optional[int]) -> int:
    ...
    ``CORVEX_CURVE_FIT_WORKERS`` > 1. Values <1 are clamped to 1.
    """
    if value is None:
        env = os.environ.get("CORVEX_CURVE_FIT_WORKERS")
        if env:
            try:
                value = int(env)
            except ValueError:
    ...
    for r in data.get("results") or []:
        ...
        if dp and Path(str(dp)).is_file():
            try:
                ...
                from corvex.skills._shared.curve_fitting_tools import load_curve_data
...
class SeriesScoutController:
    ...
    def _create_overlay_plot(
        ...
        to `corvex.utils.curve_preview.render_curve_overlay` so the lit-
...
<body>
    <div class="container">
        ...
        {candidates_html}
        {caveats_html}
        <div class="footer">Generated by Corvex Curve Fitting Analysis Agent</div>
    </div>
</body>
</html>"""
...
<body>
    <div class="container">
        ...
        {claims_html}
        {caveats_html}
        <div class="footer">Generated by Corvex Curve Fitting Series Analysis Agent</div>
    </div>
</body>
</html>"""
