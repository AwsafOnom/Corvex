...
def _extract_document_text(path: Path, ocr_model: Any = None) -> Dict[str, Any]:
    ...
    Thin wrapper over the shared ``corvex.parsers.extract_text`` (which adds
    ...
    to the caller.
    """
    from corvex.parsers import extract_text
    ...
    for domain, names in list_all_skills().items():
        ...
        for name in names:
            try:
                ...
                # `skill=` until reviewed/promoted (see corvex memory).
...
class AnalysisOrchestratorTools:
    ...
    def _parse_nexus(nx_root) -> "dict | None":
        ...
        Corvex's canonical schema is the job of ``convert_metadata``,
    ...
    def _register_all_tools(self):
                    ...
                    if "sandbox" in error_msg.lower() or "declined" in error_msg.lower():
                        ...
                        return json.dumps({
                            ...
                            "recommendation": (
                                ...
                                "Please run Corvex in one of the following:\n"
        ...
        def reconcile_series(profile_analysis: int = None,
            ...
            from corvex.skills._shared._reconcile import reconcile_analysis_dirs
        ...
        def finalize_reconcile_report(interpretation: str) -> str:
            ...
            from corvex.skills._shared._reconcile import render_reconcile_report
            try:
        ...
        def run_dft_workflow(structure_description: str = None,
            ...
            try:
                ...
                if vasp_generator_method == "atomate2":
                    hint = "pip install ase pymatgen atomate2  (or: pip install 'corvex[sim]')"
                else:
        ...
        def synthesize_knowledge(analysis_ids: list, focus: str, synthesis_type: str = "reference") -> str:
            ...
            """
            from corvex.knowledge import synthesize_knowledge as _synthesize
        ...
        def graduate_to_skill(knowledge_id: str, skill_name: str, domain: str = "curve_fitting") -> str:
            ...
            # Write to the persistent store (~/.corvex/graduated_skills),
