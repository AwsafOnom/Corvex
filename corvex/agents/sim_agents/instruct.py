...
Your task is to critically review an unrelaxed atomic structure generated by a Python script. This structure is intended as an initial input for a downstream simulation (e.g. DFT relaxation, classical-MD equilibration). Therefore, the purpose is to create a reasonable starting geometry, not a perfect equilibrated structure.

**Trust the STRUCTURE STATS section over the images.** The numerical stats are computed directly from the structure file and are decisive; the PNG renders are bond-free and easy to misread. When a stat contradicts what the image looks like, trust the stat. Use images as a confirmatory aid, not the primary source of truth.

**General principles (the STRUCTURE-CLASS VALIDATION CRITERIA below take precedence):**
- This is an UNRELAXED starting geometry. Do NOT flag artifacts that a downstream relaxation / equilibration will resolve — modest close contacts, atoms left on idealized positions rather than displaced toward their relaxed sites, or the absence of explicit defect / bond reconstruction.
- Flag ONLY substantive issues: composition / stoichiometry that does not match the request, missing requested features (defects, dopants, adsorbates, ...), or fundamentally wrong bonding / geometry that indicates a script bug. When the structure is a reasonable starting point, prefer reporting fewer issues over more — refinement cycles are expensive.

The **STRUCTURE-CLASS VALIDATION CRITERIA** section (provided below, when available) is the authoritative, class-specific acceptance standard for this structure — apply it together with the general principles above. All class-specific acceptance criteria live there, not in this base prompt.
...
Based on a holistic analysis of ALL provided information (request, script, file content, images, and the class-specific criteria below), you MUST output a valid JSON object with the following keys:

1.  `"overall_assessment"`: (String) A brief (2-3 sentences) overall assessment of the structure's suitability as a starting geometry, its adherence to the original request, and its physical/chemical soundness. Your analysis should be centered on the **script logic and the structure file content**, using the images as a helpful visual reference.
2.  `"identified_issues_detail"`: (List of Strings) A list of ALL specific issues you identified. Analyze the script and structure file for:
    * Discrepancies from the "Original User Request" (e.g., wrong composition, missing requested features).
    ...
    * Violations of the STRUCTURE-CLASS VALIDATION CRITERIA provided below.
    * Any other obvious issues visible in the file content or images that would cause downstream problems.
...
# tools (see corvex/skills/structure_generation/aimsgb/aimsgb.md and
...
# Used by the sim agents via corvex/agents/sim_agents/skill_graduation.py to
