    ...
    def __init__(
        ...
        Args:
            api_key: API key for the LLM provider. When ``base_url`` is
                ...
                ``CORVEX_API_KEY``). When ``base_url`` is unset, the
        ...
        if base_url:
            ...
            if not api_key:
                raise ValueError(
                    ...
                    "CORVEX_API_KEY in the environment or pass api_key."
                )
...
def _snapshot_run_outputs(output_dir: str, skill: Optional[str]) -> Dict[str, Any]:
    ...
    :func:`corvex.skills._shared._registry.get_tool_function`. The
    callable lives alongside its skill markdown (e.g.
    ``corvex/skills/periodic_dft/vasp/vasp_output.py``) and owns its own
