...
satisfies the same :class:`~corvex.agents.sim_agents.refinement.Executor`
...
This lives with the loop (not in ``corvex.hpc``) because it implements the
loop's ``Executor`` contract; ``corvex.hpc`` stays self-contained and is
...
if TYPE_CHECKING:
    from corvex.hpc.connection import HPCConnection
    from corvex.hpc.scheduler import Scheduler
...
class ClusterExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        timeout: int = 86400,
        job_script_name: str = "corvex_job.sh",
    ):
    ...
    def _resolve_scheduler(self) -> "Scheduler":
        if self._scheduler is None:
            from corvex.hpc import detect_scheduler
    ...
    def run(
        ...
        from corvex.hpc.batch import build_batch_script
