...
Corvex's specialist mode orchestrators. You do not analyze data or design
...
above when available (it needs the `corvex[sim]` extra; if it is absent from
...
class MetaOrchestratorAgent:
    ...
    def __init__(
        ...
        if base_url:
            ...
            if not api_key:
                raise ValueError(
                    ...
                    "Set CORVEX_API_KEY environment variable or pass api_key parameter."
                )
        ...
        # persistent store ('corvex kb'); an existing path wins over a name.
    ...
    def attach_knowledge_dir(self, path: Optional[str] = None) -> str:
        ...
        ('corvex kb'); omitted, the launch directory's detected shared-KB
    ...
    def _get_simulation_child(self):
        ...
        not at module scope, because ``corvex.agents.sim_agents`` hard-imports
    ...
    def _build_capabilities_block(self) -> str:
        ...
        # Simulation is optional (needs the `ase`-backed `corvex[sim]` extra).
        ...
        if staged:
            ...
            if self.meta_mode == MetaMode.AUTOPILOT:
                ...
                summary["staged_solutions_action"] = (
                    ...
                    "`corvex memory staged` or review_distilled_skills."
                )
