...
``from corvex.agents.skills.<...> import ...`` imports even when the
canonical path is ``corvex.skills.<...>`` — a natural hallucination
...
This shim eagerly walks the ``corvex.skills`` package tree at import
time and registers every submodule under the equivalent
``corvex.agents.skills.*`` alias in ``sys.modules``. Both paths
...
import corvex.skills as _root


_PREFIX = "corvex.agents.skills."
_ROOT_PKG = "corvex.skills"


def _walk_and_alias() -> None:
    # Alias the root itself first so 'corvex.agents.skills' is the same
    # module as 'corvex.skills'.
    sys.modules.setdefault("corvex.agents.skills", _root)
    ...
    # Skip-the-domain aliases: ``corvex.agents.skills.<skill>`` →
    # ``corvex.skills.<domain>.<skill>``. The LLM frequently elides the
    # domain layer (e.g. it tries ``corvex.agents.skills.xrd.<tool>``
    # rather than ``corvex.agents.skills.structure_matching.xrd.<tool>``).
