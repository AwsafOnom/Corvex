...
By default Corvex records nothing about individual LLM calls — no prompts, responses,
...
Enable in code::
    ...
    import corvex
    corvex.enable_tracing("run/llm_trace.jsonl")
    ...
    corvex.disable_tracing()

or via the environment (auto-enabled on first call)::

    export CORVEX_TRACE_FILE=run/llm_trace.jsonl
...
def is_enabled() -> bool:
    """Whether a trace destination is active (set via enable_tracing or $CORVEX_TRACE_FILE)."""
    return _active_path() is not None


def _active_path() -> Optional[str]:
    """Resolve the active trace path, honoring $CORVEX_TRACE_FILE on first use."""
    global _trace_path, _env_checked
    if _trace_path is None and not _env_checked:
        _env_checked = True
        env = os.environ.get("CORVEX_TRACE_FILE")
        if env:
            enable_tracing(env)
    return _trace_path
