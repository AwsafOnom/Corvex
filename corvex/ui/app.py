"""Corvex Streamlit UI — single-page app."""
...
from corvex.ui.state import init_session_state, ChatTask, FeedbackRequest
from corvex.ui.components.sidebar import render_sidebar, start_session
from corvex.ui.components.chat_uploads import render_pre_chat_uploads
from corvex.ui.components.file_viewer import render_file_preview
from corvex.ui.components.tools_agents import render_tools_agents_tab
from corvex.ui.components.skills import render_skills_tab
from corvex.ui._features import simulate_enabled
from corvex.ui.output_capture import AgentStoppedError, OutputCapture
from corvex.ui.theme import inject_theme
from corvex.ui.config import AVATAR_USER, AVATAR_AGENT, APP_MODES, SESSION_DIR_PREFIXES
...
_LOGO_DARK = _LOGO_DIR / "corvex_logo_v3_dark.svg"
_LOGO_LIGHT = _LOGO_DIR / "corvex_logo_v3_light.svg"

st.set_page_config(
    page_title="Corvex — Agentic Science",
    page_icon=str(_LOGO_DIR / "corvex_favicon.svg"),
    layout="wide",
)
...
from corvex.ui import registry as _session_registry  # noqa: E402
...
def _find_new_md_documents() -> list[str]:
    ...
    try:
        from corvex.agents.planning_agents.user_interface import (
            load_deliverables)
...
def _run_agent_chat(task: ChatTask, agent, user_input: str) -> None:
    ...
    from corvex import hitl as _hitl
    ...
    from corvex.utils.log_context import effective_thread, keep_in_fanout_panel
...
if not st.session_state.agent_initialized:
    ...
    if _pending is not None or _pending_resume is not None:
        ...
        with col_c:
            st.markdown(
                ...
                'background:#8B9CF5;animation:init-pulse 1.4s ease-in-out infinite"></div>'
                '<div style="width:10px;height:10px;border-radius:50%;'
                'background:#8B9CF5;animation:init-pulse 1.4s ease-in-out infinite 0.2s"></div>'
                '<div style="width:10px;height:10px;border-radius:50%;'
                'background:#8B9CF5;animation:init-pulse 1.4s ease-in-out infinite 0.4s"></div>'
                '</div>'
                '<p style="color:#8B9CF5;font-size:1.2em;font-weight:500;margin:0">'
                f'{"Restoring session..." if _pending_resume else "Initializing agent..."}</p>'
                '<p style="color:#6B7585;font-size:0.9em;margin-top:8px">'
        ...
        if _pending_resume is not None:
            from corvex.ui.components.sidebar import resume_session
    ...
    with col_c:
        ...
        st.markdown(
            f'<p style="text-align:center;color:#6B7585;font-size:0.85em;'
        ...
        if _logo.exists():
            ...
            if _is_dark:
                st.markdown(
                    ...
                    'transparent 0deg,transparent 270deg,#2E3640 300deg,'
                    '#8B9CF5 330deg,#E8BC4A 345deg,#8B9CF5 355deg,transparent 360deg);'
            ...
            st.title("Corvex")
        st.markdown(
            '<p style="text-align:center;color:#8892A0;font-size:1.1em;'
            'margin-top:12px;margin-bottom:24px">'
            "Agentic workflows for analysis, planning, and simulation</p>",
            unsafe_allow_html=True,
        )
        # ── Simulate mode: pre-session connection status ─────────
        if st.session_state.app_mode == "simulate":
            st.markdown(
                '<p style="text-align:center;color:#6B7585;font-size:0.95em;'
            ...
            if _conn and _conn.is_connected:
                ...
                st.markdown(
                    '<p style="text-align:center;color:#6B7585;font-size:0.85em;'
                    'margin-bottom:16px">'
...
if st.session_state.app_mode == "simulate" and simulate_enabled():
    ...
    from corvex.ui.components.simulations import render_simulations_tab
    ...
    with chat_tab:
                    ...
                    if p.exists():
                        ...
                        with st.expander(f"{_doc_title}: {p.name}"):
                            with st.container(height=600):
                                ...
                                from corvex.ui.md_images import (
                                    inline_local_images)
        ...
        def _agent_monitor():
            ...
            if not task.is_running and (task.result is not None or task.error is not None):
                ...
                # the timestamp label.
                try:
                    from corvex.ui.session_meta import (
    ...
    def _discover_sessions(current_session_dir: str | None) -> list[tuple[str, Path]]:
        ...
        from corvex.ui.session_meta import session_label
        for s in sessions:
    ...
    if telemetry_tab is not None:
        from corvex.ui.components.telemetry import render_telemetry_tab
        with telemetry_tab:
