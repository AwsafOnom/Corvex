...
def _render_hpc_connection() -> None:
    ...
    from corvex.hpc.scheduler import detect_scheduler
    ...
    with st.expander(label, expanded=False):
        if is_connected:
            ...
            from corvex.hpc.scheduler import JobStatus
            n_active = sum(
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        elif _env_src.get("api_key") == "CORVEX_API_KEY" and not base_url:
            # Proxy key was prefilled but no base URL is set — the proxy path
            # needs one, and vendors reject the proxy key without it.
            st.caption("⚠️ Proxy key detected — set a Base URL (or export `CORVEX_BASE_URL`) to use it.")
        ...
        if not consent:
            with st.expander("What does the agent execute?"):
                from corvex.executors import LLM_EXECUTION_DESCRIPTION
                ...
                st.warning(
                    ...
                    "Example: `docker run -p 8501:8501 corvex-ui`"
                )
        # ── Resume past session ────────────────────────────
        if not _locked:
...
def start_session(model: str, api_key: str, base_url: str, mode: str, fh_api_key: str = "", mp_api_key: str = "", embedding_model: str = None, embedding_api_key: str = None) -> None:
    ...
    """
    import corvex
    import corvex.executors as executors
    ...
    if mp_api_key:
        corvex.set_api_key('materials_project', mp_api_key)
...
def _collect_restored_deliverables(session_path: Path) -> tuple:
    ...
    """
    from corvex.agents.planning_agents.user_interface import load_deliverables
...
def _reset_session() -> None:
    ...
    try:
        from corvex.ui import registry as _session_registry
