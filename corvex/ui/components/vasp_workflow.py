...
from corvex.ui.components.sim_workflow import (
    _render_monitoring,
    _render_results,
    _upload_text,
    _q,
)
from corvex.ui.components.wizard_state import (
    apply_vasp_defaults,
    save_vasp,
)
...
def _render_configure() -> None:
    ...
    _has_key = bool(
        cfg.get("api_key")
        or os.environ.get("CORVEX_API_KEY")
    ...
    if not _has_key:
        st.warning(
            ...
            "sidebar with a key set, or export CORVEX_API_KEY before starting."
        )
    ...
    if has_hpc:
        ...
        if env is None:
            from corvex.hpc.probe import probe_remote
    ...
    remote_work_dir = st.text_input(
        ...
        placeholder="/scratch/$USER/corvex/vasp_runs",
    ...
    with sc3:
        job_name = st.text_input(
            "Job name (-J)", value="corvex_vasp", key="vasp_slurm_jname"
        )
    ...
    with c_submit:
        if st.button(
            ...
            try:
                ...
                job_name = st.session_state.get("hpc_gen_job_name", "corvex_vasp")
                job = HPCJob(
                    job_id=job_id,
                    name=job_name,
