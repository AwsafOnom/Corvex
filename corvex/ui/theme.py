...
def _inject_js(html: str) -> None:
    ...
    if not _JS_DEGRADED_NOTICE_SHOWN:
        ...
        print(
            f"Corvex: Streamlit {st.__version__} rejected the "
...
.stButton > button[kind="primary"] {
    background-color: #4F6BEB !important;
...
.stButton > button[kind="primary"]:hover {
    background-color: #6B84F5 !important;
    box-shadow: 0 2px 8px rgba(79, 110, 235, 0.35);
}
...
.agent-spinner-container {
    ...
    gap: 12px;
    padding: 12px 16px;
    background: linear-gradient(135deg, #1A1F28 0%, #13161B 100%);
    border: 1px solid #2E3640;
    border-left: 3px solid #E8BC4A;
    border-radius: 6px;
}
.agent-spinner-dot {
    width: 8px;
    ...
    animation: corvex-pulse 1.4s ease-in-out infinite;
    font-size: 0;
    line-height: 0;
    overflow: hidden;
}
...
[data-testid="stToggle"] [role="checkbox"][aria-checked="true"] {
    background-color: #E8BC4A !important;
}
...
.stTabs [data-testid="stExpander"] .stButton > button[kind="secondary"]:hover {
    background-color: #2E3848 !important;
...
.stButton > button[kind="primary"]:hover {
    background-color: #6B84F5 !important;
    box-shadow: 0 2px 8px rgba(79, 110, 235, 0.25);
}
/* Mode selector — pull buttons up */
div:has(> [data-testid="stMarkdown"] .mode-selector-anchor) {
    margin-bottom: -2rem !important;
...
div:has(> [data-testid="stMarkdown"] .mode-selector-anchor) + div .stButton > button[kind="secondary"] {
    ...
    color: #4F6BEB !important;
    border: 1.5px solid #4F6BEB !important;
    font-weight: 600;
}
div:has(> [data-testid="stMarkdown"] .mode-selector-anchor) + div .stButton > button[kind="secondary"] * {
    color: #4F6BEB !important;
...
div:has(> [data-testid="stMarkdown"] .mode-selector-anchor) + div .stButton > button[kind="secondary"]:hover {
    ...
    color: #4F6BEB !important;
    border-color: #6B84F5 !important;
    box-shadow: 0 2px 8px rgba(79, 110, 235, 0.15);
}
section[data-testid="stSidebar"] .stButton > button {
    background-color: #4F6BEB;
    color: #FFFFFF !important;
...
[data-testid="stChatInput"] textarea:focus {
    border-color: #4F6BEB;
    box-shadow: 0 0 4px rgba(79, 110, 235, 0.2);
    caret-color: #4F6BEB !important;
}
/* ── Password visibility toggle ────────────────────── */
.stTextInput button svg {
    fill: #8892A0 !important;
    stroke: #8892A0 !important;
    color: #8892A0 !important;
}
.stTextInput button {
    color: #8892A0 !important;
    ...
    function styleWidgets(){
        ...
        doc.querySelectorAll('button[kind="secondary"]').forEach(function(btn){
            ...
            if (tip.indexOf('Stop') >= 0 || txt === '\\u25A0') {
                ...
                btn.style.setProperty('border', '1px solid #E6E9EF', 'important');
