"""Shared constants and defaults for the Corvex Streamlit UI."""
...
# Vendor formats that Corvex itself cannot read; surfaced in the upload
...
def extra_data_extensions_for(agent) -> tuple:
    ...
    connected, so the uploader does not advertise formats Corvex cannot
    actually handle.
    """
    if agent is None:
        return ()
...
def resolve_prefill(
    ...
    Main-key precedence:
      ...
      1. Proxy pair — ``CORVEX_API_KEY`` when a base URL is available
         (``CORVEX_BASE_URL`` env or one already entered): a fully-configured
         proxy deployment.
      2. The vendor key matching the selected model's provider
         (``ANTHROPIC_API_KEY`` / ``OPENAI_API_KEY`` / ``GEMINI_API_KEY`` …).
      3. ``CORVEX_API_KEY`` on its own — a proxy deployment whose base URL will
         be supplied separately (the sidebar warns that one is still needed).
      4. Otherwise empty — no mismatched key is substituted.

    ``base_url`` prefills from ``CORVEX_BASE_URL`` when set; FutureHouse /
