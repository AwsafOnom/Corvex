...
order:
  ...
  1. ``CORVEX_SIMULATE`` env var, if set — ``0/false/no/off`` forces
...
into ``corvex[sim]``. ``paramiko`` has no such cross-ecosystem
overlap in our user base.

Note: the result is cached for the process lifetime (``@lru_cache``).
Changing ``CORVEX_SIMULATE`` requires restarting Streamlit.
"""
...
def simulate_enabled() -> bool:
    override = os.environ.get("CORVEX_SIMULATE", "").strip().lower()
