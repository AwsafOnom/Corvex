...
INTERNAL_PROXY_KEY = 'CORVEX_API_KEY'
...
INTERNAL_PROXY_BASE_URL = 'CORVEX_BASE_URL'
...
class APIKeyManager:
    ...
    def show_status(self):
        ...
        if proxy_key:
            ...
            print(f"  ✓ {'CORVEX_API_KEY':18} {masked:20} (for --base-url)")
        else:
            print(f"  ✗ {'CORVEX_API_KEY':18} {'Not set':20} (for --base-url)")
    ...
    def __init__(self, service: str, additional_note: Optional[str] = None):
        suggestions = {
            'google': [
                ...
                "Configure in code: corvex.set_api_key('google', 'your-key')",
                "Get your key at: https://aistudio.google.com/apikey"
            ],
            'openai': [
                "Set environment variable: export OPENAI_API_KEY='your-key'",
                "Configure in code: corvex.set_api_key('openai', 'your-key')",
                "Get your key at: https://platform.openai.com/api-keys"
            ],
            'anthropic': [
                "Set environment variable: export ANTHROPIC_API_KEY='your-key'",
                "Configure in code: corvex.set_api_key('anthropic', 'your-key')",
                "Get your key at: https://console.anthropic.com/settings/keys"
            ],
            'futurehouse': [
                "Set environment variable: export FUTUREHOUSE_API_KEY='your-key'",
                "Configure in code: corvex.set_api_key('futurehouse', 'your-key')",
                "Get your key at: https://platform.futurehouse.org/"
            ],
            'materials_project': [
                "Set environment variable: export MP_API_KEY='your-key'",
                "Configure in code: corvex.set_api_key('materials_project', 'your-key')",
                "Get your key at: https://next-gen.materialsproject.org/api"
            ],
            'proxy': [
                "Set environment variable: export CORVEX_API_KEY='your-key'",
        ...
        tips = suggestions.get(service.lower(), [
            ...
            f"Configure in code: corvex.set_api_key('{service}', 'your-key')"
        ])
...
def require_vendor_credentials(model_name: str) -> None:
    ...
    expected vendor env var(s) AND -- when ``CORVEX_API_KEY`` is set without
    a ``base_url`` -- explains that ``CORVEX_API_KEY`` is the proxy key, not
    a vendor credential.
    """
    ...
    if get_internal_proxy_key():
        note = (
            "CORVEX_API_KEY is currently set, but it's the proxy key — vendors "
            "reject it on the direct LiteLLM path. To use the internal proxy "
            "instead, pass `base_url=` alongside CORVEX_API_KEY."
        )
