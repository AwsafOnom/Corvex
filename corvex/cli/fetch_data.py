"""``corvex fetch-xrd-library`` — install the prebuilt XRD fingerprint library.

One-time download of the COD-derived reference library (several hundred MB)
into the persistent per-user store (``~/.corvex/xrd_fingerprints/``), enabling
...
def main() -> int:
    from corvex.skills.structure_matching.xrd.fingerprint import (
        DEFAULT_LIBRARY_URL, fetch_fingerprint_library, _default_store_path)

    ap = argparse.ArgumentParser(
        prog="corvex fetch-xrd-library",
