#!/usr/bin/env python3
"""
corvex plan - Interactive Planning Orchestrator
...
def main():
    """Main entry point for 'corvex plan' command"""
    
    parser = argparse.ArgumentParser(
        prog='corvex plan',
        description='Corvex Planning Orchestrator - Interactive AI Research Agent',
...
Examples:
  ...
  corvex plan --autonomy autopilot --data-dir ./experimental_results
  
  # Full autonomous mode (no human review)
  corvex plan --autonomy autonomous --data-dir ./data --knowledge-dir ./papers --code-dir ./code
  
  # Use a different model
  corvex plan --model gemini-2.0-flash
  
  # Use Claude
  corvex plan --model claude-sonnet-4-20250514
  
  # Use OpenAI
  corvex plan --model gpt-4o
  
  # Use internal proxy with custom endpoint
  corvex plan --base-url https://my-proxy.example.com/v1 --model my-model

  # Save to a named session directory
  corvex plan --session-dir ./my_campaign
...
Environment Variables:
  CORVEX_API_KEY          API key for internal proxy
  ...
  OpenAI:    gpt-4o, gpt-4-turbo, o1-preview, etc.
    ...
    parser.add_argument(
        ...
        help='Papers/literature directory, OR the name of a knowledge '
             'base from the persistent store (see: corvex kb list)'
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'Example: corvex plan --tools ./custom_stats_tools.py'
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'Example: corvex plan --mcp stdio:fs:npx,-y,@modelcontextprotocol/server-filesystem,/tmp'
        )
    )
    ...
    if args.autonomy in ('autopilot', 'autonomous') and not args.data_dir:
        parser.error(
            ...
            f"Example: corvex plan --autonomy {args.autonomy} --data-dir ./experimental_results"
        )
    ...
    if args.knowledge_dir:
        from corvex.knowledge.kb_store import resolve_knowledge_source
        try:
...
""")
        ...
        if not api_key:
            if base_url:
                # Internal proxy
                api_key = os.getenv("CORVEX_API_KEY")
                if not api_key:
                    print(f"\n⚠️  No CORVEX_API_KEY found in environment.")
                    print(f"   When using --base-url, set CORVEX_API_KEY for authentication.")
                    api_key = input(f"Enter your proxy API key (CORVEX_API_KEY): ").strip()
                    if not api_key:
    ...
    def handle_command(self, user_input: str) -> bool:
        ...
        if cmd == "/help":
            ...
            from corvex.agents.planning_agents.planning_orchestrator import AutonomyLevel
                ...
                if new_level:
                    ...
                    if new_level in (AutonomyLevel.AUTOPILOT, AutonomyLevel.AUTONOMOUS):
                        if not self.data_dir:
                            ...
                            print(f"   For best results, restart with: corvex plan --autonomy {new_level.value} --data-dir ./your_data")
