#!/usr/bin/env python3
"""
corvex explore - Interactive Meta-Agent Orchestrator

The meta-agent is Corvex's default entry point (bare `corvex`): a single
...
def main():
    """Main entry point for the 'corvex explore' command (and bare 'corvex')."""

    parser = argparse.ArgumentParser(
        prog='corvex explore',
        description='Corvex Meta-Agent - one chat surface that auto-routes '
                    'to the analyze and plan specialists',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Launch the meta-agent (identical to running bare `corvex`)
  corvex explore

  # Seed the first turn with your research goal
  corvex explore --message "Analyze grains.tif then plan a follow-up campaign"

  # Autopilot mode (meta delegates on its own judgement; reports as it goes)
  corvex explore --mode autopilot

  # Use a different model / an internal proxy
  corvex explore --model gemini-2.0-flash
  corvex explore --base-url https://my-proxy.example.com/v1 --model my-model

  # Share custom skills / tools / MCP servers with every specialist
  corvex explore --skills ./raman_skill.md ./xrd_skill.md
  corvex explore --tools ./image_tools.py
  corvex explore --mcp stdio:fs:npx,-y,@modelcontextprotocol/server-filesystem,/tmp
  ...
  FUTUREHOUSE_API_KEY      FutureHouse API key (enables literature search)
    ...
    parser.add_argument('--knowledge-dir', type=str, dest='knowledge_dir',
                        help='Stable knowledge/KB for planning delegations: '
                             ...
                             'from the persistent store (corvex kb list). '
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'corvex explore --skills ./raman_skill.md ./ftir_skill.md'
        )
    )
...
class MetaPlayground:
    ...
    def setup(self):
        ...
        print("\n" + "=" * 60)
        print("🧭  CORVEX META-AGENT")
        print("=" * 60)
        print("""
One chat surface that coordinates Corvex's specialists for you:
        ...
        if not api_key:
            if base_url:
                ...
                if not api_key:
                    ...
                    api_key = input("Enter your proxy API key (CORVEX_API_KEY): ").strip()
                    if not api_key:
