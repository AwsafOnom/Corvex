#!/usr/bin/env python3
"""
corvex simulate - Interactive Simulation Orchestrator
...
def main():
    """Main entry point for 'corvex simulate' command."""

    parser = argparse.ArgumentParser(
        prog='corvex simulate',
        description='Corvex Simulation Orchestrator - Interactive VASP DFT input prep',
...
Examples:
  ...
  corvex simulate --request "Build a rutile TiO2 supercell with one O vacancy"

  # Autopilot mode (AI proceeds with reasonable defaults; surfaces big decisions)
  corvex simulate --mode autopilot

  # Use a different model
  corvex simulate --model gemini-2.0-flash

  # Internal proxy
  corvex simulate --base-url https://my-proxy.example.com/v1 --model my-model

Modes (matching `corvex analyze` / `corvex plan` for consistent UX):
...
class SimulatePlayground:
    ...
    def setup(self):
        ...
        print("\n" + "=" * 60)
        print("⚛️  CORVEX SIMULATION ORCHESTRATOR (VASP DFT prep)")
        print("=" * 60)
        print("""
...
""")
    ...
    def handle_command(self, user_input: str):
        ...
        if cmd.startswith("/mode"):
            from corvex.agents.sim_agents.simulation_orchestrator import SimulationMode
            parts = cmd.split()
            if len(parts) == 1:
