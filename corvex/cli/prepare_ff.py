#!/usr/bin/env python3
"""
corvex prepare-ff - AMBER Force Field Preparation
...
def main():
    """Main entry point for 'corvex prepare-ff' command."""

    parser = argparse.ArgumentParser(
        prog="corvex prepare-ff",
        description="Prepare AMBER force field parameters for LAMMPS simulations",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Simple peptide in vacuum
  corvex prepare-ff --pdb peptide.pdb --goal "Study conformational dynamics"

  # Protein solvated in water with ions
  corvex prepare-ff --pdb protein.pdb --goal "Protein folding" --solvate --neutralize

  # Custom output directory and water model
  corvex prepare-ff --pdb system.pdb --goal "Binding study" \\
      --output-dir ./my_sim --water-model opc --protein-ff ff19SB

  # With non-standard ligand
  corvex prepare-ff --pdb complex.pdb --goal "Drug binding" \\
      --ligand ligand.pdb --ligand-name LIG --ligand-charge -1 --solvate

  # Generate test system (alanine dipeptide)
  corvex prepare-ff --test --goal "Test AMBER pipeline"

  # Generate test system with solvation
  corvex prepare-ff --test --solvate --goal "Test solvated system"

  # Check if AmberTools are available
  corvex prepare-ff --check-tools
...
Requirements:
  ...
  LLM API key: Set CORVEX_API_KEY, GOOGLE_API_KEY, or use --api-key
...
Environment Variables:
  ...
  ANTHROPIC_API_KEY   Anthropic API key
    ...
    # ── Resolve API key ───────────────────────────────────────────
    api_key = args.api_key or os.environ.get("CORVEX_API_KEY") \
    ...
    if not api_key:
        parser.error(
            "No API key found. Set CORVEX_API_KEY, GOOGLE_API_KEY, "
            "or use --api-key"
        )
...
def _check_tools() -> int:
    ...
    from corvex.skills.force_field.amber.amber import check_amber_tools
