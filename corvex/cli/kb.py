#!/usr/bin/env python3
"""corvex kb — manage named, reusable knowledge bases.

Named KBs live under the persistent Corvex home
(``~/.corvex/knowledge_bases/<name>/``; ``CORVEX_HOME`` relocates them)
and are usable from any launch directory:

    corvex plan --knowledge-dir <name>
    corvex explore --knowledge-dir <name>
...
def main():
    from corvex.knowledge import kb_store

    parser = argparse.ArgumentParser(
        prog="corvex kb",
        description="Manage named, reusable knowledge bases "
                    "(stored under the persistent Corvex home).",
    )
    ...
    try:
        if args.action == "list":
            ...
            if not kbs:
                print("No named knowledge bases. Create one with "
                      "'corvex kb create <name> --from <docs...>'.")
                return 0
            for m in kbs:
        ...
        if args.action == "create":
            ...
            print(f"   Use it: corvex plan --knowledge-dir {args.name}")
            return 0
