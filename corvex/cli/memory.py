#!/usr/bin/env python3
"""
corvex memory - Manage Corvex's persistent memory

The persistent store (``~/.corvex/graduated_skills``, relocatable via
``CORVEX_HOME``) holds graduated and auto-distilled skills. It survives a
...
from corvex.skills._shared._memory import (
    list_memory,
    show_memory,
    promote_memory,
    prune_memory,
...
def _cmd_memory_toggle(args) -> int:
    """`corvex memory enable|disable|status` — the persistent-memory master switch."""
    from corvex.skills import loader
    if args.action == "status":
        on = loader.memory_enabled()
        env = os.environ.get("CORVEX_MEMORY", "").strip()
        src = f"env CORVEX_MEMORY={env!r}" if env else f"config ({loader._config_path()})"
    ...
    if os.environ.get("CORVEX_MEMORY", "").strip():
        print("  ⚠️  CORVEX_MEMORY env var is set and overrides this saved setting.")
    return 0


def _cmd_list(args) -> int:
    from corvex.skills import loader
    print(f"[persistent memory: {'ON' if loader.memory_enabled() else 'OFF — inert; `corvex memory enable` to use'}]\n")
    provisional = None
...
def _cmd_demote(args) -> int:
    from corvex.skills._shared._memory import demote_memory
    domain, name = _split_ref(args.ref)
    try:
...
def _cmd_fork(args) -> int:
    """`corvex memory fork` — copy a built-in skill into the persistent store."""
    _warn_memory_off()  # a fork only SHADOWS while persistent memory is on
    from corvex.skills._shared._memory import fork_builtin
    domain, name = _split_ref(args.ref)
    try:
    ...
    print(f"   Contribute back later: `corvex memory diff-builtin {domain}/{name}`; "
          f"prune the fork once merged upstream.")
    return 0


def _cmd_diff_builtin(args) -> int:
    from corvex.skills._shared._memory import diff_builtin
    domain, name = _split_ref(args.ref)
    try:
...
def _warn_memory_off():
    ...
    if not loader.memory_enabled():
        print("⚠️  Persistent memory is OFF — this won't affect runs until you "
              "enable it (`corvex memory enable`).")
...
def _cmd_upgrade(args) -> int:
    _warn_memory_off()
    import difflib
    from corvex.agents.exp_agents.instruct import (
...
def _cmd_bank(args) -> int:
    """`corvex memory bank` — list script-bank records (episodic memory)."""
    from corvex.skills._shared import _script_bank
    ...
    if len(refs) == 1:
        ...
        if out.get("ready_to_consolidate"):
            print(f"   ✅ {out['n_staged_total']} staged for this technique — "
                  f"ready: `corvex memory consolidate {domain}/{out['technique']}`")
        else:
            print(f"   Accumulating {out['n_staged_total']}/"
                  f"{_staging.consolidate_min_n()} for consolidation "
                  f"(`consolidate` can force it now).")
    print("   Review with `corvex memory staged`; bank records are kept "
          "(marked ✓ promoted).")
    return 0


def _cmd_bank_groups(args) -> int:
    """`corvex memory bank-groups` — same-system variant clusters."""
    from corvex.skills._shared import _script_bank
    ...
    for dom in domains:
        ...
        for g in _script_bank.find_variant_groups(dom, threshold=thr):
            ...
            print(f"    → promote together: `corvex memory bank-promote {refs} "
...
def main():
    """Entry point for 'corvex memory'."""
    parser = argparse.ArgumentParser(
        prog="corvex memory",
        description="Manage Corvex's persistent memory (graduated + auto-distilled skills).",
