"""MCP server that exposes Corvex's analysis and planning tools.

Allows any MCP client (Claude Desktop, Cursor, etc.) to use Corvex as a
tool provider.  Start with::

    corvex serve --model gemini-3.1-pro-preview
...
- **co-pilot** — tools that need approval return a ``needs_input``
  response; the MCP client must call ``corvex_respond`` to continue.

The ``mcp`` package is installed by default with Corvex.
"""
...
def _require_mcp():
    if not HAS_MCP:
        raise ImportError(
            ...
            "Install with: pip install corvex"
        )
...
def _openai_to_mcp_tool(schema: dict, prefix: str = "corvex") -> types.Tool:
    ...
    if name in _BACKGROUND_CAPABLE_TOOLS:
        ...
        schema_copy.setdefault("properties", {})["background"] = {
            ...
            "description": (
                ...
                "immediately. Use corvex_job_status and corvex_job_result "
...
# ── Corvex branding for MCP clients ───────────────────────────────────
# For a general audience it must be obvious where the client agent ends and
# Corvex begins. Three text-surface layers (plus the server/tool `icons`
# below, rendered as a real logo by GUI clients): the server identity, a
# "Corvex · <Tool Name>" display title on every tool, and a marker on each
# streamed narration line.

CORVEX_MARK = "🔬 Corvex"
_LOGO_PATH = Path(__file__).parent / "ui" / "assets" / "corvex_logo_v3_light.svg"
...
def _tool_title(name: str) -> str:
    """Display title: 'Corvex · Run Analysis' from 'corvex_run_analysis'."""
    base = name
    for pre in ("corvex_plan_", "corvex_"):
        if base.startswith(pre):
            base = base[len(pre):]
            break
    return "Corvex · " + base.replace("_", " ").title()
...
def _restore_jobs(state: dict) -> None:
    ...
    corvex_respond against them explains what happened instead of
    erroring as unknown."""
    path = _jobs_store_path(state)
...
class _MCPChannel:
    ...
    pending queue for a client to answer via ``corvex_respond``.
    ...
    ``corvex_job_status`` can report ``awaiting_input``.
    """
...
class _JobLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            from corvex.utils.log_context import effective_thread
...
def _execute_tool_captured(tools, tool_name: str, kwargs: dict,
    ...
    from corvex.hitl import set_thread_channel
...
def _build_approval_prompt(tool_name: str, kwargs: dict) -> str:
    ...
    parts = [f"Corvex wants to execute: {tool_name}"]
    if kwargs:
    ...
    parts.append("\nCall corvex_respond with 'yes' to approve or 'no' to cancel.")
    return "\n".join(parts)
...
def _load_shared_credentials() -> list:
    """Load ``~/.corvex/credentials.env`` (KEY=VALUE lines) into the process
    ...
    path = Path.home() / ".corvex" / "credentials.env"
    loaded = []
    if not path.exists():
        return loaded
    try:
...
# progress with no background-job dance. Disable: CORVEX_MCP_NARRATION=0.
import contextvars as _contextvars

_MCP_SESSION: "_contextvars.ContextVar" = _contextvars.ContextVar(
    "corvex_mcp_session", default=None)
...
def _narration_enabled() -> bool:
    return os.environ.get("CORVEX_MCP_NARRATION", "1").lower() not in (
        "0", "false", "no", "off")
...
async def _to_thread_streaming(fn, *args, label: str = None) -> str:
    ...
    Corvex marker, bracketed by start/finish banners, so in an interleaved
    client transcript it is obvious which output is Corvex's and which is
    ...
    async def _emit(text):
        ...
        if session is None:
            return
        try:
            await session.send_log_message("info", text, logger="corvex")
        except Exception:  # noqa: BLE001 - client gone / no support
            session = None

    await _emit(f"{CORVEX_MARK} ▶ {label or 'working'} …")
    sent = 0
    while True:
        done = task.done()
        if session is not None and len(lines) > sent:
            ...
            for line in batch:
                if not str(line).strip():
                    continue
                await _emit(f"{CORVEX_MARK} │ {str(line).rstrip()}")
                if session is None:
                    break
        if done:
            break
        await asyncio.sleep(_NARRATION_POLL_S)
    await _emit(f"{CORVEX_MARK} ✓ {label or 'done'}")
    return task.result()
...
def create_server(
    *,
    ...
    try:
        ...
        _server_version = _pkg_version("corvex")
    except Exception:  # noqa: BLE001
    ...
    server = Server(
        ...
        instructions=(
            "Corvex — autonomous scientific analysis, simulation and "
            "experiment planning. Every tool here is served by Corvex's "
            "own orchestrators and specialist agents (curve fitting, "
            "image/hyperspectral analysis, Bayesian optimization, "
            "planning).\n"
            "\n"
            "How to work with Corvex (cross-tool patterns; per-tool detail "
            "is in each tool's description):\n"
            "- Corvex interprets its own results: responses carry the "
            "specialist's analysis (detailed_analysis, key_findings, "
            "scientific_claims). Relay it, attributed to Corvex, rather "
            "than re-deriving it; add your own synthesis only across "
            "results or against the user's goals.\n"
            "- Long tasks: call with background=true, then poll "
            "corvex_job_status (log_tail = live narration; awaiting_input "
            "= a question for you, answer via corvex_respond, empty string "
            "approves; interrupted = the server restarted, state is "
            "checkpointed — re-issue the call). Fetch corvex_job_result.\n"
            "- Campaigns are stateful per session: ingested data, schema, "
            "directions and bounds persist — do not re-send what Corvex "
            "already knows, and heed the warnings in responses (Corvex "
            "says when it had to assume something and how to fix it).\n"
            "- Measurement campaigns over raw data: keep feature columns "
            "stable across rounds by passing the first analysis run's "
            "output_directory as prior_analysis_paths with "
            "reuse_locked_script=true on later runs.\n"
            "- Corvex's session directory is server-private: never write "
            "to it, and never author data files yourself — a wrong ingest "
            "is fixed by re-calling corvex_analyze_file with "
            "force_regenerate=true, not by editing files. Only tool "
            "results count as measurements or findings. Calls can take "
            "minutes; slow is normal."
        ),
        website_url="https://github.com/ziatdinovmax/Corvex",
        icons=_corvex_icons() or None,
    )
    ...
    state: Dict[str, Any] = {
        ...
        "meta_orch": None,
        # Pending items awaiting a client's corvex_respond, keyed by
    ...
    async def list_tools() -> List[types.Tool]:
        ...
        if state["analysis_orch"]:
            for schema in state["analysis_orch"].tools.openai_schemas:
                tools.append(_openai_to_mcp_tool(schema, prefix="corvex"))
        ...
        # Always include corvex_respond so it's available if the user
        # switches to co-pilot/autopilot mode mid-session via
        # corvex_set_autonomy (MCP clients only call tools/list once).
        tools.append(types.Tool(
            name="corvex_respond",
            description=(
                "Answer a pending Corvex request. Two cases: (1) a tool "
                ...
                "job runs ('awaiting_input' in corvex_job_status) — the "
                ...
                "properties": {
                    ...
                    "request_id": {
                        ...
                        "description": (
                            ...
                            "'needs_input' payload or corvex_job_status). "
        ...
        tools.append(types.Tool(
            ...
            description=(
                ...
                "Returns the new mode and whether corvex_respond is now needed."
            ),
            inputSchema={
                "type": "object",
                "properties": {
        ...
        if state["analysis_orch"]:
            tools.append(types.Tool(
                name="corvex_orchestrate_analysis",
                description=(
                    "Delegate a complete analysis workflow to Corvex's analysis "
                    ...
                    "analyses where Corvex's domain expertise should drive the "
                    ...
                    "properties": {
                        ...
                        "background": {
                            ...
                            "description": (
                                ...
                                "corvex_job_status and corvex_job_result to poll "
        ...
        if state["planning_orch"]:
            tools.append(types.Tool(
                name="corvex_orchestrate_planning",
                description=(
                    "Delegate a complete planning workflow to Corvex's planning "
                    ...
                    "complex experimental design tasks where Corvex's domain "
        ...
        if state["simulation_orch"]:
            tools.append(types.Tool(
                name="corvex_orchestrate_simulation",
                description=(
                    "Delegate a complete simulation workflow to Corvex's "
                    ...
                    "for a full task where Corvex's domain expertise should drive "
        ...
        if state["meta_orch"]:
            tools.append(types.Tool(
                name="corvex_orchestrate_meta",
                description=(
                    "Delegate a complete multi-step scientific investigation to "
                    "Corvex's META orchestrator — its orchestrator-of-"
                    ...
                    "background=true (recommended): poll corvex_job_status for "
                    "progress (log_tail) and answer its questions "
                    "(awaiting_input -> corvex_respond)."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "background": {
                            ...
                            "description": (
                                ...
                                "corvex_job_status; fetch with "
                                "corvex_job_result. Default: false."
                            ),
                        },
                    },
                    "required": ["prompt"],
            ...
            return [types.TextContent(
                ...
                text=json.dumps({
                    ...
                    "message": (
                        ...
                        "Use corvex_job_status to check progress, "
                        "then corvex_job_result to retrieve the result."
                    ),
                }),
            )]
    ...
    async def read_resource(uri: str) -> str:
        ...
        if uri == "corvex://analysis/status" and a_orch:
            return json.dumps({
        ...
        if uri == "corvex://analysis/metadata" and a_orch:
            return json.dumps(
                getattr(a_orch, "current_metadata", None) or {}, indent=2
            )

        if uri == "corvex://analysis/agents" and a_orch:
        ...
        if uri == "corvex://planning/status" and p_orch:
            return json.dumps({
        ...
        if uri == "corvex://planning/plan" and p_orch:
        ...
        if uri == "corvex://session/status" and a_orch:
            return await read_resource("corvex://analysis/status")
        if uri == "corvex://session/metadata" and a_orch:
            return await read_resource("corvex://analysis/metadata")
        if uri == "corvex://session/agents" and a_orch:
            return await read_resource("corvex://analysis/agents")
...
def _init_orchestrators(state: dict, config: dict) -> None:
    ...
    if not session_dir:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        session_dir = str(Path.home() / "corvex_mcp_sessions" / f"session_{ts}")
    ...
    if config["mode"] in ("analyze", "both"):
        from corvex.agents.exp_agents.analysis_orchestrator import (
    ...
    if config["mode"] == "simulate":
        ...
        # without it must not break `corvex serve` — it just won't offer the
        # simulate mode.
        try:
            from corvex.agents.sim_agents.simulation_orchestrator import (
    ...
    if config["mode"] == "meta":
        ...
        # analysis/planning tool sets, mirroring how bare `corvex`
        ...
        from corvex.agents.meta_agent.meta_orchestrator import (
...
def _handle_job_status(
    ...
    # with the parked question(s) so the client can corvex_respond.
    if job["status"] == "running":
        ...
        if questions:
            ...
            payload["message"] = (
                ...
                "corvex_respond(request_id=..., response=...) — empty "
    ...
    if not future.done():
        return [types.TextContent(
            ...
            text=json.dumps({
                "status": "running",
                "job_id": job_id,
                "message": "Job is still running. Check again later with corvex_job_status.",
            }),
        )]
...
def _execute_meta_chat_captured(orch, prompt: str, autonomy_str: str,
    ...
    """
    from corvex.agents.meta_agent.meta_orchestrator import MetaMode
    from corvex.hitl import set_thread_channel
        ...
        return [types.TextContent(
            ...
            text=json.dumps({
                ...
                "message": (
                    ...
                    "Use corvex_job_status to watch progress (log_tail) and "
                    "answer its questions; corvex_job_result for the result."
                ),
            }),
        )]
...
async def _handle_orchestrate(
    ...
    orch_key = {
        "corvex_orchestrate_analysis": "analysis_orch",
        "corvex_orchestrate_planning": "planning_orch",
        "corvex_orchestrate_simulation": "simulation_orch",
    ...
    run_in_background = arguments.pop("background", False)
    tool_label = name.replace("corvex_", "")
...
def _handle_set_autonomy(
    ...
    return [types.TextContent(
        ...
        text=json.dumps({
            ...
            "corvex_respond_needed": new_mode != "autonomous",
        }),
    )]
        ...
        return [types.TextContent(
            ...
            text=json.dumps({
                ...
                "message": ("Answer delivered; the run resumes. Poll "
                            "corvex_job_status for progress if this was "
                            "a background job."),
            }),
        )]
...
def run_sse(server: Server, host: str = "127.0.0.1", port: int = 8000) -> None:
    ...
    logging.info(f"Corvex MCP server (SSE) at http://{host}:{port}/sse")
