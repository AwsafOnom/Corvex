# Connecting Corvex to agent clients — one guide for all of them

Corvex serves its orchestrators and specialist agents over the
...
- **Credentials live with the server, once.** Put `KEY=VALUE` lines (e.g.
  ...
  `AWS_REGION_NAME=...`) in `~/.corvex/credentials.env` *of the machine or
  container that runs `corvex serve`*. The server loads it at startup
...
- **Paths are server-side.** Every `file_path` / `data_path` you pass to a
  `corvex_*` tool must be a path the *server* process can open. Same
...
- **Generated code runs on the server.** Corvex executes LLM-written
...
- **While tools run**, the server streams narration to the client as MCP
  log notifications (lines marked `🔬 Corvex │ …`); GUI clients that
  support MCP icons also show the Corvex logo on the server and every
  tool. Terminal clients that render neither can watch the server's own
  terminal/log.

## Three ways to run the server

**A. Spawned per session (stdio).** The client launches `corvex serve`
itself. Generate the config instead of writing it:

```bash
corvex serve --print-mcp-json --mode both --session-dir ~/corvex_sessions/mycampaign
```
...
nothing else), or a path to this installation's `corvex` otherwise.
...
```bash
corvex serve --mode both --transport sse --host 127.0.0.1 --port 8000 \
              --session-dir ~/corvex_sessions/mycampaign
# clients use: {"type": "sse", "url": "http://127.0.0.1:8000/sse"}
```

`corvex serve --print-mcp-json --transport sse --port 8000` prints that
client entry too.
...
RUN useradd --create-home corvex
USER corvex
RUN pip install --no-cache-dir --user "corvex>=0.0.65" uvicorn starlette sse-starlette
ENV PATH="/home/corvex/.local/bin:${PATH}" UNSAFE_EXECUTION_OK=true
VOLUME /data
EXPOSE 8000
ENTRYPOINT ["corvex", "serve"]
CMD ["--mode", "both", "--transport", "sse", "--host", "0.0.0.0", \
     "--port", "8000", "--session-dir", "/data/session"]
```

Mount the credentials file at `/home/corvex/.corvex/credentials.env`
...
|---|---|
| **Claude Code** (CLI) | `claude mcp add corvex -- <command …>` or paste the entry into `.mcp.json` at the project root |
| **Claude Desktop** | `~/Library/Application Support/Claude/claude_desktop_config.json` (macOS) / `%APPDATA%\Claude\claude_desktop_config.json` (Windows); renders the Corvex logo |
...
| **deepagents / LangChain (library)** | `MultiServerMCPClient({"corvex": <entry>})` + `load_mcp_tools(session)` — hold one session per campaign; see the deepagents-corvex examples repo |
| **Deep Agents CLI (managed cloud)** | `deepagents mcp-servers add --url <PUBLIC-URL> --name corvex` — the URL must be reachable from the hosted runtime and MUST sit behind auth (`--header KEY=VALUE`); never expose the raw SSE port |

Give the client agent domain instructions (an `AGENTS.md` / system prompt)
covering the campaign protocol — background jobs + `corvex_job_status`
polling + `corvex_respond` for questions, explicit
...
The deepagents-corvex repo ships a worked example.

## Container layouts (who runs where)

|  | Corvex spawned (stdio) | Corvex on host (SSE) | Corvex in container (SSE) |
|---|---|---|---|
| **Client on host** | default — recipe A | recipe B | recipe C with `ports: 127.0.0.1:8000:8000`; client config same as B |
| **Client in a container** | install `corvex` in the *same* image and spawn over stdio — one shared boundary, simplest | `http://host.docker.internal:8000/sse` — works, but the client's container isolates it *from* the host while depending on a host service; rarely the right shape | **recommended**: both services on one compose network |
...
services:
  corvex:
    build: {context: ., dockerfile: Dockerfile.corvex}
    volumes: ["./data:/data", "~/.corvex:/home/corvex/.corvex:ro"]
    # no ports: — reachable only on this network
  agent:
    build: {context: ., dockerfile: Dockerfile.agent}
    environment: [CORVEX_URL=http://corvex:8000/sse]
    volumes: ["./data:/data:ro"]     # read datasets; write nothing of the server's
    depends_on: [corvex]
```

The agent reaches Corvex at the service name (`http://corvex:8000/sse`);
...
if an ingest looks wrong, the fix is re-calling `corvex_analyze_file`
...
- Interactive daily use, want to watch Corvex work → **B** (or the
  `corvex-dcode`-style launcher pattern).
- Production, shared lab server, CI, or anything multi-user → **C**, one
  corvex service per concurrent campaign.
