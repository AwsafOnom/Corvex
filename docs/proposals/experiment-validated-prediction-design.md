...
- **Not** "Corvex autonomously grounds the fix in literature." Edison-based
...
- **Not** full autonomy on the fix. Per the use-case framing (max quality *with
  human feedback*), Corvex catches and recommends; the human approves the
  corrected model; Corvex re-validates. That is the version that works.
...
5. **Fix** — Corvex recommends a reparameterization; human approves; rerun.
...
prediction from a model that fails the measured observables; Corvex catches
...
parameter set (human-supplied) or a Corvex-recommended one the human approves —
