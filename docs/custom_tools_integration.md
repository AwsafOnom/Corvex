# Using Corvex with Your Own Tools

If you already have Python code for a task — a denoiser, a particle detector, a custom morphology metric, a calibration routine — you can plug it into Corvex as a tool the analysis orchestrator can call during a chat session. Your function shows up alongside built-ins like `examine_data` and `run_analysis`, and the LLM will use it when the conversation calls for it.

This guide walks through how to wrap a function as a Corvex tool, how to register it (CLI, UI, or Python), and how its output flows into the built-in agents.
...
```bash
corvex analyze --tools ./my_tools.py

# Multiple files are fine
corvex analyze --tools ./preprocess.py ./morphology.py
```
...
```python
from corvex.agents.exp_agents.analysis_orchestrator import AnalysisOrchestratorAgent
...
For an interactive REPL, wrap `orch.chat()` in your own input loop — that's exactly what the `corvex analyze` CLI does internally.
...
```bash
corvex analyze --tools ./preprocess.py
```
...
Custom tools are the lightest extension. Corvex also supports two heavier ones:
...
```bash
corvex analyze --tools ./preprocess.py --agents ./my_agent.py --skills ./raman.md
```

In the web UI, custom tools upload via the **Tools** tab and custom skills via the **Skills** tab; custom agents are CLI/API only. The flags are not analyze-only either: `corvex plan`, `corvex simulate`, and `corvex explore` also accept both `--tools` and `--skills`.
