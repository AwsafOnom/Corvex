# Connecting Corvex to External MCP Servers

Corvex can connect to external MCP (Model Context Protocol) servers and use their tools alongside its built-in analysis and planning tools. This lets you extend Corvex with capabilities like literature search, database queries, file system access, or domain-specific tools — without writing any code.

## How it works

When you connect an MCP server, Corvex:
...
corvex analyze --mcp sse:myserver:http://localhost:8000/sse

# Streamable HTTP transport (remote/hosted servers)
corvex analyze --mcp http:myserver:https://host/mcp

# JSON config file
corvex analyze --mcp /path/to/mcp_config.json

# Multiple servers
corvex analyze --mcp stdio:arxiv:python,-m,arxiv_mcp_server stdio:fs:npx,-y,@modelcontextprotocol/server-filesystem,/tmp
```
...
```python
from corvex.agents.planning_agents.planning_orchestrator import PlanningOrchestratorAgent
...
The LLM will use the arXiv MCP tools to search and retrieve papers, then use Corvex's built-in tools to analyze your data — combining literature context with experimental analysis.
...
```bash
# CLI
corvex plan --mcp stdio:OpentronsAI:npx,mcp-remote,https://opentrons-opentronsai-mcp-server.hf.space/gradio_api/mcp/
```
...
The `mcp` package is included in the base Corvex installation — no extra flags needed.
