...
```
Tier 1  pip install corvex[structure-matching]     search/simulate/score + fingerprint tools
Tier 2  corvex fetch-xrd-library                   one-time reference-library download
...
```bash
pip install "corvex[structure-matching]"
```
...
| `CORVEX_LOCAL_CIF_DIR` | a local CIF directory as a search backend |
...
1. **Fetch the prebuilt artifact** (recommended; one-time, a few hundred MB):
   ...
   ```bash
   corvex fetch-xrd-library
   ```

   Installs to `~/.corvex/xrd_fingerprints/cod_fingerprints.parquet`
...
2. **Point at a shared copy** (HPC / group installs):
   ...
   ```bash
   export CORVEX_XRD_FINGERPRINT_DB=/shared/data/cod_fingerprints.parquet
   ```
...
3. **Build your own** — from the COD mirror or ANY CIF collection (private
   ...
   python -c "from corvex.skills.structure_matching.xrd.fingerprint import \
...
`CORVEX_XRD_FINGERPRINT_DB` → the per-user store. A missing library raises
...
```bash
conda create -n corvex python=3.12
conda activate corvex
conda install -c conda-forge fortran-compiler meson ninja
pip install "corvex[structure-matching]"   # BEFORE GSAS-II: settles the numpy ABI
...
| numpy ABI errors importing GSAS-II | GSAS-II was compiled against a different numpy than the one now installed. Fix: install corvex (which pins the scientific stack) **before** GSAS-II; reinstall GSAS-II after major numpy changes. |
...
GSAS-II is deliberately **not a corvex extra**: it is not on PyPI, and PyPI
rejects package metadata that carries a direct URL dependency (so a `[gsas]`
extra would block publishing corvex itself). It must be installed manually
...
3. Update `DEFAULT_LIBRARY_URL` / `DEFAULT_LIBRARY_SHA256` in
   `corvex/skills/structure_matching/xrd/fingerprint.py`.
