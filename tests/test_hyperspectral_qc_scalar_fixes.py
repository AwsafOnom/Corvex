...
from corvex.agents.exp_agents.controllers import hyperspectral_controllers as hc
from corvex.agents.exp_agents.controllers.hyperspectral_controllers import (
...
def test_write_results_file_emits_scalar_columns(tmp_path, monkeypatch):
    ...
    from corvex.agents.exp_agents.hyperspectral_analysis_agent import (
        HyperspectralAnalysisAgent,
    )
    from corvex.agents.exp_agents.feature_table import write_feature_table
...
def test_panel_value_space_overlay_not_mirrored(tmp_path):
    ...
    from corvex.skills.hyperspectral.eels.eels import create_fit_examples_panel
    e = 151
...
def test_robust_hist_window_excludes_and_counts_outliers():
    from corvex.skills.hyperspectral.eels.eels import _robust_hist_window
...
def test_dashboard_renders_with_extreme_outliers():
    from corvex.skills.hyperspectral.eels.eels import create_feature_dashboard
    m = RNG.randn(64, 64)
...
def test_normalize_to_uint8_renders_tiny_range_with_contrast():
    from corvex.skills._shared.image_processor import _normalize_to_uint8
...
def test_load_image_raw_preserves_native_values(tmp_path):
    from corvex.skills._shared.image_processor import load_image
    topo = _stm_like_topo()
    p = tmp_path / "topo.npy"
    np.save(p, topo)
...
def test_reflection_receives_metadata_scalars_and_aux():
    ...
    from corvex.agents.exp_agents.controllers.base_controllers import (
