...
def test_a_refined_portfolio_does_not_serve_stale_directions():
    ...
    from corvex.agents.planning_agents.parser_utils import resync_portfolio
...
def test_no_new_direct_readers_of_the_experiment_schema():
    ...
    root = Path("corvex/agents/planning_agents")
    ...
    for other in (Path("corvex/agents/meta_agent"), Path("corvex/ui")):
        if not other.exists():
            continue
