...
data files need stem-matched .json sidecars for datasets 2 and 3):
    ...
    CORVEX_411_THERMAL_FILE=...   CORVEX_411_THERMAL_META=...
    CORVEX_411_DIFFRACTION_DATA=... CORVEX_411_SPECTROSCOPY_DATA=...
...
def stage_data() -> tuple[Path, Path, Path, Path, list[str]]:
    thermal_file = Path(os.environ["CORVEX_411_THERMAL_FILE"])
    thermal_meta = Path(os.environ["CORVEX_411_THERMAL_META"])
    diff_spec = os.environ["CORVEX_411_DIFFRACTION_DATA"]
    spec_spec = os.environ["CORVEX_411_SPECTROSCOPY_DATA"]
...
def main() -> int:
    for var in ("AWS_BEARER_TOKEN_BEDROCK", "CORVEX_411_THERMAL_FILE",
                "CORVEX_411_THERMAL_META", "CORVEX_411_DIFFRACTION_DATA",
                "CORVEX_411_SPECTROSCOPY_DATA"):
