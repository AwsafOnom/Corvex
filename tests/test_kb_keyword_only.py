...
from corvex.knowledge.knowledge_base import KnowledgeBase
...
def test_content_gates_count_chunks_not_vectors():
    ...
    pa = Path("corvex/agents/planning_agents/planning_agent.py").read_text()
    assert pa.count("keyword-only KBs count as content") == 3
    ot = Path("corvex/agents/planning_agents/orchestrator_tools.py").read_text()
