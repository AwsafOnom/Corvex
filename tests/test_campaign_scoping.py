...
from corvex.agents.planning_agents.planning_agent import (
    PlanningAgent,
    objectives_share_campaign,
)
from corvex.agents.planning_agents.base_agent import BaseAgent
from corvex.agents.planning_agents import planning_agent as pa_mod
from corvex.agents.planning_agents.orchestrator_tools import (
    OrchestratorTools,
...
def test_expand_is_safe_on_uncompacted_and_broken_states():
    ...
    broken = {"current_plan": {"literature_search": "__corvex_lit_ref__:dead"},
              "plan_history": []}
    out = expand_planner_state(broken)
    assert out["current_plan"]["literature_search"] == "__corvex_lit_ref__:dead"


def test_agent_state_file_round_trips_through_compaction(tmp_path):
    from corvex.agents.planning_agents.planning_agent import PlanningAgent
    from corvex.agents.planning_agents.base_agent import BaseAgent
...
def test_saved_files_land_in_the_active_delegation_dir(tmp_path):
    ...
    from corvex.agents.planning_agents import orchestrator_tools as ot
...
def test_ui_embeds_marked_deliverables_and_skips_bulk(tmp_path, monkeypatch):
    ...
    import ast
    from corvex.agents.planning_agents.user_interface import record_deliverable
    ...
    src = Path("corvex/ui/app.py").read_text()
    tree = ast.parse(src)
...
def test_paths_are_plain_and_pasteable(monkeypatch):
    ...
    from corvex.agents.planning_agents import user_interface as ui
...
def test_lab_plan_report_is_marked_a_deliverable(tmp_path):
    ...
    src = Path("corvex/agents/planning_agents/orchestrator_tools.py").read_text()
...
def test_missing_metadata_prompt_degrades_without_a_human(monkeypatch, capsys,
    ...
    import builtins
    from corvex.agents.planning_agents import user_interface as ui
    from corvex.agents.planning_agents.parser_utils import (
