...
from corvex.agents.sim_agents.utils import (
...
def test_detect_packmol_requires_wrapper_and_binary():
    ...
    with patch(
        "corvex.agents.sim_agents.utils.importlib.util.find_spec",
        _fake_find_spec({"pymatgen", "pymatgen.io.packmol"}),
    ), patch("corvex.agents.sim_agents.utils.shutil.which", return_value=None):
...
def test_detect_packmol_available_when_both_present():
    with patch(
        ...
        "corvex.agents.sim_agents.utils.shutil.which",
...
def test_detect_fail_closed_on_probe_error():
    ...
    with patch(
        "corvex.agents.sim_agents.utils.importlib.util.find_spec", _boom
    ), patch("corvex.agents.sim_agents.utils.shutil.which", return_value=None):
        avail = detect_structure_build_tools()
