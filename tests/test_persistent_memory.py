...
Covers:
  - the loader's discovery of the persistent graduated-skills store
    (precedence ordering, de-dup, CORVEX_HOME relocation);
...
from corvex.skills import loader
...
def _enable_memory(monkeypatch):
    ...
    monkeypatch.setenv("CORVEX_MEMORY", "1")
from corvex.skills._shared._graduation import (
...
class TestSkillRootsDiscovery:
    def test_home_redirect_via_env(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
    ...
    def test_graduated_dir_appears_in_roots_after_creation(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        monkeypatch.delenv("CORVEX_SKILLS_PATH", raising=False)
    ...
    def test_precedence_order_env_then_graduated_then_builtin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CORVEX_SKILLS_PATH", str(env_root))
    ...
    def test_dedupes_when_home_also_in_skills_path(self, tmp_path, monkeypatch):
        ...
        (gd / "curve_fitting" / "y" / "y.md").write_text("---\ndescription: g\n---\n## overview\ng\n")
        monkeypatch.setenv("CORVEX_HOME", str(home))
        # Point CORVEX_SKILLS_PATH at the same graduated dir.
        monkeypatch.setenv("CORVEX_SKILLS_PATH", str(gd))
        roots = [r.resolve() for r in loader._skill_roots()]
...
class TestImportSafety:
    def test_graduation_helper_is_ase_free(self):
        ...
        script = textwrap.dedent(
            ...
            from corvex.skills._shared._graduation import graduate_to_skill_file
            from corvex.skills import loader
    ...
    def test_sim_shim_reexports_helper(self):
        from corvex.agents.sim_agents.skill_graduation import (
            graduate_to_skill_file as shim_fn,
        )
        from corvex.skills._shared._graduation import graduate_to_skill_file as real_fn
...
class TestRoutingFilter:
    ...
    def test_provisional_excluded_but_loadable(self, tmp_path, monkeypatch):
        ...
        from corvex.agents.exp_agents.analysis_orchestrator_tools import (
...
class TestT2StageHook:
    ...
    def test_stages_solution_no_skill_written(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        from corvex.agents.exp_agents.curve_fitting_agent import CurveFittingAgent
        from corvex.skills._shared import _memory, _staging
    ...
    def test_opt_out_via_env(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        monkeypatch.setenv("CORVEX_T2_AUTODISTILL", "0")
        from corvex.agents.exp_agents.curve_fitting_agent import CurveFittingAgent
        agent = self._fake_agent(str(tmp_path), self._Model())
        assert CurveFittingAgent._maybe_stage_t2_solutions(agent, self._hot_state()) == []
...
class TestMetaReviewTool:
    ...
    def _tools(self):
        import types
        from corvex.agents.meta_agent.meta_orchestrator_tools import MetaOrchestratorTools
    ...
    def test_list_show_promote_discard(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        from corvex.skills._shared import _memory
...
class TestHotDeviationStamp:
    def test_curve_fitting_stamp(self):
        from corvex.agents.exp_agents.controllers.curve_fitting_controllers import (
    ...
    def test_image_stamp(self):
        from corvex.agents.exp_agents.controllers.image_analysis_controllers import (
...
class TestImageT2Stage:
    ...
    def test_stages_solution_no_skill_written(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        from corvex.agents.exp_agents.image_analysis_agent import ImageAnalysisAgent
        from corvex.skills._shared import _memory, _staging
        agent = self._fake_agent(str(tmp_path), self._Model())
...
class TestStaging:
    ...
    def test_crud_and_grouping(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        from corvex.skills._shared import _staging
    ...
    def test_safe_path_component(self):
        from corvex.skills._shared._graduation import safe_path_component
    ...
    def test_ephemeral_store_warning(self, monkeypatch, caplog):
        import logging
        import corvex.skills._shared._graduation as g
        # CORVEX_HOME set -> never warn, even if "in a container"
        monkeypatch.setenv("CORVEX_HOME", "/data")
        ...
        # In a container, no CORVEX_HOME -> warn ONCE
        monkeypatch.delenv("CORVEX_HOME", raising=False)
        g._EPHEMERAL_WARNED = False
        caplog.clear()
        with caplog.at_level(logging.WARNING):
            g.warn_if_ephemeral_store()
...
class TestMemorySwitch:
    def test_default_off(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        monkeypatch.delenv("CORVEX_MEMORY", raising=False)  # no env, no config
    ...
    def test_set_persists_and_env_overrides(self, tmp_path, monkeypatch):
        ...
        assert loader.memory_enabled() is False
        monkeypatch.setenv("CORVEX_MEMORY", "1")   # env overrides config(False)
        assert loader.memory_enabled() is True
        monkeypatch.setenv("CORVEX_MEMORY", "0")
        loader.set_memory_enabled(True)
...
class TestDemote:
    ...
    def test_promote_demote_round_trip(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        from corvex.skills._shared import _memory
        from corvex.skills.loader import graduated_skills_dir
...
def _has_fork_features() -> bool:
    from corvex.skills._shared import _memory as _m
...
class TestAdditivityGuard:
    def test_regression_warnings(self):
        from corvex.skills._shared._staging import _regression_warnings
        existing = ("## overview\nlong text " + "word " * 100
                    + "\n## planning\nrules\n## validation\nchecks\n")
        clean = existing + "\nnew lesson appended\n"
    ...
    def test_preserve_structure_round_trip(self):
        from corvex.skills._shared._staging import _preserve_structure
        existing = (
    ...
    def test_preserve_structure_reinstates_dropped_section(self):
        from corvex.skills._shared._staging import (
...
class TestT2BankUnification(TestT2StageHook):
    def test_hot_win_promotes_bank_record(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        from corvex.agents.exp_agents.curve_fitting_agent import CurveFittingAgent
        from corvex.skills._shared import _script_bank as sb, _staging
    ...
    def test_hot_win_fallback_when_bank_disabled(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORVEX_HOME", str(tmp_path))
        monkeypatch.setenv("CORVEX_SCRIPT_BANK", "0")  # bank off, memory on
        from corvex.agents.exp_agents.curve_fitting_agent import CurveFittingAgent
        from corvex.skills._shared import _staging
