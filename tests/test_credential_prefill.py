...
from corvex import auth
from corvex.ui.config import (
    resolve_prefill, reconcile_autofill, resolve_embedding_prefill,
)


_RELEVANT_VARS = [
    "CORVEX_API_KEY", "CORVEX_BASE_URL",
...
def test_get_internal_proxy_base_url(clean_env):
    ...
    clean_env.setenv("CORVEX_BASE_URL", "https://proxy.example/v1")
    assert auth.get_internal_proxy_base_url() == "https://proxy.example/v1"
    assert auth.INTERNAL_PROXY_BASE_URL == "CORVEX_BASE_URL"
...
def test_proxy_pair_used_when_base_url_in_env(clean_env):
    """CORVEX_API_KEY + CORVEX_BASE_URL → proxy key fills the main field."""
    clean_env.setenv("CORVEX_API_KEY", "proxy-key")
    clean_env.setenv("CORVEX_BASE_URL", "https://proxy/v1")
    out = resolve_prefill("claude-opus-4-6")
    assert out["api_key"] == ("proxy-key", "CORVEX_API_KEY")
    assert out["base_url"] == ("https://proxy/v1", "CORVEX_BASE_URL")
...
def test_proxy_key_used_when_base_url_already_entered(clean_env):
    ...
    assert out["api_key"] == ("proxy-key", "CORVEX_API_KEY")
    # No CORVEX_BASE_URL env → base_url field itself is not prefilled.
    assert out["base_url"] == ("", None)
...
def test_base_url_prefilled_independently_of_proxy_key(clean_env):
    """CORVEX_BASE_URL set without CORVEX_API_KEY: base_url is still surfaced,
