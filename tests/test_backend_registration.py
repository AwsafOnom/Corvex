...
def test_entry_point_loader_imported():
    ...
    not raise."""
    from corvex.skills.structure_matching._backends import _load_entry_points

    _load_entry_points()  # Idempotent; Corvex itself declares an empty group.
...
def test_entry_point_load_failure_logged_not_raised(caplog):
    ...
    with patch.object(_md, "entry_points", lambda *a, **kw: [_BrokenEP()]), \
         caplog.at_level(logging.WARNING,
                         logger="corvex.skills.structure_matching._backends"):
        _load_entry_points()
