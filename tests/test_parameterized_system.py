...
``corvex[ff]`` extra and verified live elsewhere; here we assert the contract
...
from corvex.agents.sim_agents._parameterized_system import (
    ComponentSpec,
    ParameterizedSystem,
)
from corvex.agents.sim_agents import _engine_inputs
...
def test_write_md_inputs_fails_loudly_without_payload(tmp_path):
    # Either openff is absent (ImportError naming corvex[ff]) or present but the
