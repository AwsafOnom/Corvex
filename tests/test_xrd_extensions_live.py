...
from corvex.skills.structure_matching._backends.materials_project import (
    MP_API_AVAILABLE,
)
from corvex.skills.structure_matching.xrd.simulate_xrd import PYMATGEN_XRD_AVAILABLE
from corvex.skills.structure_matching.xrd.score_match_robust import PULP_AVAILABLE
...
requires_deps = pytest.mark.skipif(
    not _DEPS_OK,
    reason="pymatgen XRD + pulp required (pip install corvex[structure-matching])",
)
...
def test_anonymous_formula_filter_offline(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CORVEX_LOCAL_CIF_DIR", str(cif_dir))
