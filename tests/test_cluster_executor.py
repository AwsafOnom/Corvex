...
# corvex.hpc.* imports paramiko (an optional dependency) via hpc/__init__, so
# skip the whole module cleanly where it isn't installed rather than erroring.
pytest.importorskip("paramiko")

from corvex.agents.sim_agents.cluster_executor import (  # noqa: E402
    ClusterExecutor, _RC, _STDOUT, _STDERR,
)
from corvex.hpc.batch import build_batch_script  # noqa: E402
from corvex.hpc.scheduler import (  # noqa: E402
...
class FakeScheduler:
    ...
    def batch_directives(self, resources):
        return [f"#FAKE job={resources.get('job_name', 'corvex')}"]
...
class TestClusterExecutorRun:
    def test_full_flow_uploads_submits_polls_downloads(self, tmp_path):
        ...
        assert "/scratch/production/corvex_job.sh" in uploaded
        # Submitted with the job script + remote work dir.
        assert sched.submitted == [("corvex_job.sh", "/scratch/production")]
    ...
    def test_connect_factory_builds_profile_and_connects(self, monkeypatch):
        ...
        import corvex.hpc.connection as conn_mod
        captured = {}
    ...
    def test_uploaded_script_has_directives_and_wrapper(self, tmp_path):
        ...
        ex.run({"in.lj": "S"}, "lmp -in in.lj", str(tmp_path / "run"))
        script = conn._remote["/remote/home/run/corvex_job.sh"]
        assert script.startswith("#!/bin/bash")
        assert "#FAKE job=corvex" in script
