...
from corvex.skills.structure_matching._backends import StructureCandidate
from corvex.skills.structure_matching.xrd.search_structures import (
    TOOL_SPEC,
    _candidate_to_dict,
    _dedupe,
    search_structures,
)
from corvex.skills.structure_matching.xrd.simulate_xrd import (
...
_skip_no_xrd = pytest.mark.skipif(
    ...
    reason="pymatgen XRD analysis module not installed; pip install corvex[structure-matching]",
)
from corvex.skills.structure_matching.xrd.score_match_fast import (
...
def test_search_local_only_returns_matching_candidates(tmp_path):
    ...
    monkey_env = {"CORVEX_LOCAL_CIF_DIR": str(cif_dir)}
...
def test_search_cell_only_blind_query(tmp_path):
    ...
    with patch.dict("os.environ", {"CORVEX_LOCAL_CIF_DIR": str(cif_dir)}):
        result = search_structures(
...
def test_queryspec_cell_only_validation():
    from corvex.skills.structure_matching._backends._base import QuerySpec
...
def test_cod_query_volume_center():
    ...
    from corvex.skills.structure_matching._backends.cod import _query_volume_center
...
def test_search_multi_chemistry_warns_per_backend_failure(tmp_path):
    ...
    from corvex.skills.structure_matching._backends import (
    ...
    with patch.object(LocalCIFBackend, "query", selective_raise), \
         patch.dict("os.environ", {"CORVEX_LOCAL_CIF_DIR": str(cif_dir)}):
        result = search_structures(
...
@patch("corvex.skills.structure_matching._backends.materials_project.MPRester")
@patch(
    "corvex.skills.structure_matching._backends.materials_project.MP_API_AVAILABLE",
    True,
)
def test_search_dedupes_mp_and_local_same_phase(mock_mprester, tmp_path):
    ...
    with patch.dict("os.environ", {
        "MP_API_KEY": "fake",
        "CORVEX_LOCAL_CIF_DIR": str(cif_dir),
    }):
        result = search_structures(
            query={"chemistry": ["Si"]},
            sources=["mp", "local"],
