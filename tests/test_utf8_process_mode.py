...
# corvex/cli/__init__.py does `from .main import main`, which rebinds the
# attribute `main` on the package to the FUNCTION — so both
# `from corvex.cli import main` and `import corvex.cli.main as m` hand back
# the function, not the module. Go through importlib to get the module.
cli_main = importlib.import_module("corvex.cli.main")


PACKAGE = pathlib.Path(__file__).resolve().parents[1] / "corvex"
...
def test_no_text_write_in_the_package_omits_its_encoding():
    ...
    (or use corvex.utils.text_io) and this passes.
    """
...
def utf8_probe(monkeypatch):
    ...
    monkeypatch.delenv("CORVEX_UTF8_RESTARTED", raising=False)
    return calls
...
def test_restarts_under_utf8_on_a_cp1252_locale(monkeypatch, utf8_probe):
    ...
    monkeypatch.setattr(sys, "argv", ["corvex", "plan", "--data-dir", "x"])
    ...
    assert utf8_probe["env"]["CORVEX_UTF8_RESTARTED"] == "1"
    ...
    assert utf8_probe["env"]["CORVEX_ARGV0"] == "corvex"
...
def test_does_not_restart_twice(monkeypatch, utf8_probe, capsys):
    ...
    _pretend_locale(monkeypatch, "cp1252")
    monkeypatch.setenv("CORVEX_UTF8_RESTARTED", "1")
    cli_main.ensure_utf8_mode()  # must return, not exit
    assert "cmd" not in utf8_probe
    captured = capsys.readouterr()
    # stdout is the MCP transport under `corvex serve` — warn on stderr only.
...
def test_restored_argv0_keeps_the_program_name_in_usage(monkeypatch):
    """The child restores argv[0] so `--help` still says "corvex"."""
    monkeypatch.setenv("CORVEX_ARGV0", "corvex")
    monkeypatch.setattr(sys, "argv", ["/site-packages/corvex/cli/main.py", "help"])
    _pretend_locale(monkeypatch, "cp1252", utf8_mode=1)  # already restarted
    import io, contextlib
    with contextlib.redirect_stdout(io.StringIO()):
        cli_main.main()
    assert sys.argv[0].startswith("corvex")
