...
from corvex.agents.sim_agents.simulation_orchestrator import (  # noqa: E402
    SimulationMode,
)
...
SimStub.__module__ = "corvex.agents.sim_agents.simulation_orchestrator"
...
def test_orchestrate_simulation_requires_simulate_mode():
    ...
    (content,) = asyncio.run(_handle_orchestrate(
        state, "corvex_orchestrate_simulation", {"prompt": "x"}, None))
...
def test_orchestrate_simulation_background_job():
    ...
    (content,) = asyncio.run(_handle_orchestrate(
        state, "corvex_orchestrate_simulation",
...
def test_simulate_mode_registers_sim_tools_end_to_end(tmp_path):
    ...
    its tools with clean corvex_ names — standalone (no analysis/planning leak),
    ...
    assert {"corvex_run_simulation", "corvex_generate_structure",
            "corvex_route_simulation", "corvex_orchestrate_simulation",
            "corvex_analyze_output", "corvex_list_available_software"} <= names
    assert all(not n.startswith("corvex_sim_") for n in names)   # no collision prefix
    # generic control/job tools come for free
    assert {"corvex_job_status", "corvex_job_result",
            "corvex_respond"} <= names
    # standalone: analysis/planning tool sets are NOT registered
    assert "corvex_analyze_file" not in names
    assert "corvex_run_optimization" not in names
    # a long sim tool got the optional background param injected
    rs = next(t for t in tools if t.name == "corvex_run_simulation")
