...
def test_kill_switch(monkeypatch):
    monkeypatch.setenv("CORVEX_BANK_EDIT_ADAPT", "0")
    fake, _ = make_self([GOOD_REPLY])
...
def test_score_override(monkeypatch):
    monkeypatch.setenv("CORVEX_BANK_EDIT_ADAPT_SCORE", "0.95")
    fake, _ = make_self([GOOD_REPLY])
...
def test_template_contract():
    from corvex.agents.exp_agents.instruct import BANK_EDIT_ADAPT_INSTRUCTIONS
...
def test_image_wrapper_shares_the_core():
    from corvex.agents.exp_agents.controllers.image_analysis_controllers \
...
def hs_fake(model_replies, task_success=True):
    from corvex.agents.exp_agents.controllers.hyperspectral_controllers \
...
def test_hs_supplied_script_mode_in_run_attempt_source():
    src = Path("corvex/agents/exp_agents/controllers/"
...
def test_single_shared_implementation():
    from pathlib import Path
    curve = Path("corvex/agents/exp_agents/controllers/"
                 "curve_fitting_controllers.py").read_text()
    image = Path("corvex/agents/exp_agents/controllers/"
                 "image_analysis_controllers.py").read_text()
    hs = Path("corvex/agents/exp_agents/controllers/"
              "hyperspectral_controllers.py").read_text()
    engine = Path("corvex/agents/exp_agents/_qc_engine.py").read_text()
