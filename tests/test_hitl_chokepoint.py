"""Phase-0 chokepoint tests: corvex.hitl routing + the no-raw-input gate."""
...
from corvex import hitl
from corvex.hitl import (
    ConsoleChannel,
    FeedbackRequest,
...
CORVEX_ROOT = Path(hitl.__file__).parent
...
def test_get_user_feedback_routes_through_chokepoint():
    from corvex.agents.planning_agents import user_interface
...
def test_no_raw_input_outside_allowed_files():
    offenders = []
    for py in CORVEX_ROOT.rglob("*.py"):
        rel = py.relative_to(CORVEX_ROOT).as_posix()
    ...
    assert offenders == [], (
        ...
        "corvex.hitl.request_human_feedback instead"
    )
