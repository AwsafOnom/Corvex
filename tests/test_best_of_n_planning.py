...
from corvex.agents.planning_agents.user_interface import display_plan_candidates
...
def test_resolve_n_candidates_default_policy():
    ...
    from corvex.agents.planning_agents.orchestrator_tools import resolve_n_candidates
...
def _load_ui_parser():
    ...
    src = Path(__file__).resolve().parents[1] / "corvex" / "ui" / "app.py"
...
def _capture_ctx(monkeypatch):
    ...
    from corvex.agents.planning_agents import planning_rag as pr
    seen = {}
...
def _summary(plan, ideation, capsys):
    from corvex.agents.planning_agents.user_interface import display_plan_summary
...
def test_ideation_override_bars_invented_optimization_params():
    ...
    from corvex.agents.planning_agents.instruct import (
...
def test_reasoning_passes_see_every_direction():
    ...
    from corvex.agents.planning_agents.planning_rag import summarize_experiment
...
def test_dossier_renders_the_flagship_as_shipped_not_as_authored(tmp_path):
    ...
    from corvex.agents.planning_agents.orchestrator_tools import OrchestratorTools
...
def test_html_report_renders_the_portfolio(tmp_path):
    ...
    from corvex.agents.planning_agents.html_generator import HTMLReportGenerator
