...
Requires ANTHROPIC_API_KEY (or CORVEX_TEST_API_KEY + CORVEX_TEST_MODEL).
Run all:
...
from corvex.agents.planning_agents.bo_agent import BOAgent

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------

_MODEL = os.environ.get("CORVEX_TEST_MODEL", "claude-opus-4-6")
_API_KEY = (
    os.environ.get("CORVEX_TEST_API_KEY")
...
def main(argv):
    if not _API_KEY:
        print(
            "ERROR: ANTHROPIC_API_KEY (or CORVEX_TEST_API_KEY) not set.",
            file=sys.stderr,
        )
        return 2
