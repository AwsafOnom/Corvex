...
Modes (argv[1]):
  agent  — CurveFittingAgent directly with the instruction seeded in
           ...
           Set CORVEX_CROP_REPO to a repo checkout to test (e.g. a main
...
window, not the full spectrum.
    ...
    CORVEX_IR_FILE=/path/to/spectrum.csv \
...
def run_agent_mode(work: Path, src_csv: Path) -> bool:
    repo = os.environ.get("CORVEX_CROP_REPO")
    if repo:
        sys.path.insert(0, repo)
    import corvex  # noqa: F401  (resolve after path injection)
    print(f"corvex from: {corvex.__file__}")
...
def main() -> int:
    ...
    tag = os.environ.get("CORVEX_CROP_TAG", mode)
