...
Requires GEMINI_API_KEY env var (or CORVEX_TEST_API_KEY + CORVEX_TEST_MODEL).
Run with:
...
from corvex.agents.planning_agents.bo_tools import get_optimizer
from corvex.agents.planning_agents.bo_agent import BOAgent, _compute_budget_context
...
_MODEL = os.environ.get("CORVEX_TEST_MODEL", "gemini-3.1-pro-preview")
_API_KEY = os.environ.get("CORVEX_TEST_API_KEY") or os.environ.get("GEMINI_API_KEY", "")
...
if __name__ == "__main__":
    if not _API_KEY:
        print("Set GEMINI_API_KEY (or CORVEX_TEST_API_KEY) env var first.")
        sys.exit(1)
