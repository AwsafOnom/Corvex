...
from corvex.skills._shared._quality_metrics import peak_region_r2
...
def test_reexport_identity():
    ...
    from corvex.skills.curve_fitting.nmr.quality import peak_region_r2 as nmr
    from corvex.skills.curve_fitting.xrd_profile.quality import peak_region_r2 as xrd
    assert nmr is xrd is peak_region_r2


def test_tool_registered_and_skill_gated():
    from corvex.skills._shared._registry import get_tools_for, get_tool_function
...
def test_fit_pattern_reports_peak_region_r2():
    from corvex.skills.curve_fitting.xrd_profile.fit_pattern import fit_pattern
    y = _pattern(amp=200)
...
def test_frontmatter_gate_is_peak_region_r2():
    md = Path("corvex/skills/curve_fitting/xrd_profile/xrd_profile.md").read_text()
    fm = yaml.safe_load(md.split("---")[1])
    gate = fm["quality_gate"]
    assert gate["metric"] == "peak_region_r2"
    from corvex.agents.exp_agents.quality_gate import _coerce
    g = _coerce(gate)
