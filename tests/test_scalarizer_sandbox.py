...
def test_construction_without_sandbox() -> None:
    ...
    from corvex.agents.planning_agents.scalarizer_agent import ScalarizerAgent
    # Reset the global cache so this test runs in a known state regardless
    # of test ordering.
    import corvex.executors as _exec
...
def test_scalarize_returns_failure_dict_on_declined_sandbox() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with patch(
            "corvex.agents.planning_agents.scalarizer_agent.require_sandbox_approval",
            return_value=False,
        ):
