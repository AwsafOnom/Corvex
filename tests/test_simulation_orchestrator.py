...
def _get_api_key(allow_dummy: bool = False) -> str:
    """Return whichever LLM key is set, preferring CORVEX_API_KEY."""
    key = (os.environ.get("CORVEX_API_KEY")
    ...
    raise RuntimeError(
        "Set CORVEX_API_KEY or ANTHROPIC_API_KEY to run live-LLM tests."
    )
...
def test_2d_run_simulation_reuses_prebuilt_structure(model_name: str):
    ...
    """
    import corvex.agents.sim_agents.simulation_pipeline as sp
    with tempfile.TemporaryDirectory() as td:
        orch = _make_orch(model_name, td + "/sim")
...
def test_3_post_run_analysis_synthetic(model_name: str):
    ...
    """
    from corvex.skills._shared._registry import get_tool_function
...
def test_4_mode_switching(model_name: str):
    ...
    from corvex.agents.sim_agents import SimulationMode
    with tempfile.TemporaryDirectory() as td:
...
def test_6_session_persistence(model_name: str):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        from corvex.agents.sim_agents import SimulationOrchestratorAgent
...
def test_8_hpc_tools_mock_connection(model_name: str):
    ...
    from corvex.agents.sim_agents import SimulationOrchestratorAgent, SimulationMode
    from corvex.hpc.scheduler import HPCJob, JobStatus
...
def stress_4_aimsgb_skill_loaded(model_name: str):
    ...
    the built-in skill via corvex.skills.loader → curated content lands
...
#   HPC_REMOTE_DIR  remote scratch directory for test jobs (e.g. /scratch/user/corvex_test)
# ---------------------------------------------------------------------------
...
def integration_1_hpc_slurm(model_name: str):
    ...
    from corvex.hpc.scheduler import SlurmScheduler, JobStatus
    from corvex.agents.sim_agents import SimulationOrchestratorAgent, SimulationMode
    ...
    remote_dir = f"{remote_base}/corvex_integration_test"
    fake_vasp = (
        "sleep 5\n"
...
def main():
    ...
    if needs_keys:
        if not (os.environ.get("CORVEX_API_KEY") or os.environ.get("ANTHROPIC_API_KEY")):
            print("❌ Set CORVEX_API_KEY or ANTHROPIC_API_KEY for live-LLM tests")
            sys.exit(2)
    if args.integration:
