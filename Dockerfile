...
# Install the corvex package itself (without reinstalling its dependencies).
...
RUN addgroup --system corvexgroup && adduser --system --ingroup corvexgroup corvexuser

# Set the home directory for the new user.
ENV HOME=/home/corvexuser
WORKDIR /home/corvexuser

# Copy installed Python packages from the builder stage.
COPY --from=builder /usr/local/lib/python3.11/site-packages/ /usr/local/lib/python3.11/site-packages/
# Copy the installed command-line script from the builder stage.
COPY --from=builder /usr/local/bin/corvex /usr/local/bin/corvex
...
# (or set CORVEX_HOME to a mounted path).
RUN mkdir -p /home/corvexuser/.corvex
VOLUME ["/home/corvexuser/.corvex"]

# Ensure the non-root user owns all the files in its home directory.
RUN chown -R corvexuser:corvexgroup /home/corvexuser

# Switch to the non-root user. All subsequent commands will run as this user.
USER corvexuser

# Set the entrypoint to your corvex CLI tool.
ENTRYPOINT ["corvex"]
