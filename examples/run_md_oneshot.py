...
Validates that Corvex can take a natural-language goal and *actually run* a
...
science:
  ...
  Stage B (--via-meta): the same goal through the meta agent in AUTONOMOUS
    mode (bare-``corvex`` experience), delegating to the simulation child.

Environment:
  CORVEX_API_KEY    proxy key (required for the internal proxy path)
  CORVEX_BASE_URL   proxy URL (required with the -project model names)
  CORVEX_MODEL      model name (default: claude-opus-4-8-project)
  CORVEX_RUN_COMMAND  optional run-command override, e.g. 'lmp_mpi -in {script}'
...
# SMILES, box size, or component counts (Corvex decides those).
GOAL = (
...
def _creds():
    api_key = os.environ.get("CORVEX_API_KEY")
    base_url = os.environ.get("CORVEX_BASE_URL")
    model = os.environ.get("CORVEX_MODEL", "claude-opus-4-8-project")
    if not api_key or not base_url:
        sys.exit("Set CORVEX_API_KEY and CORVEX_BASE_URL (proxy) first.")
...
def stage_a(base_dir: Path) -> dict:
    ...
    run_command = os.environ.get("CORVEX_RUN_COMMAND")
    if run_command:
...
def stage_b(base_dir: Path) -> dict:
    """Through the meta agent in AUTONOMOUS — the bare-corvex experience."""
