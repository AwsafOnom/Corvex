...
def check_mace_availability():
    from corvex.skills._shared import mlip_tools
...
def run_without_mace(
    ...
    from corvex.agents.sim_agents._potential import (
    ...
    if runner == "lammps":
        from corvex.skills.molecular_dynamics.lammps import lammps
        ...
        from corvex.agents.sim_agents._ase_runner import generate_ase_script
...
def main():
    ...
    parser.add_argument(
        "--api-key", default=os.environ.get("CORVEX_API_KEY", ""),
        help="Falls back to CORVEX_API_KEY env var",
    )
    ...
    if not args.api_key:
        if mace_available:
            print("  WARNING: no CORVEX_API_KEY — LLM model selection will fail")
        else:
            print("  no CORVEX_API_KEY (fine for local-only walkthrough)")
    ...
    from corvex.agents.sim_agents.mlip_agent import MLIPAgent
