...
Run:
    export CORVEX_API_KEY=...        # or GOOGLE_API_KEY, OPENAI_API_KEY, etc.
...
from corvex.agents.sim_agents.simulation_router import (
    SimulationRouter,
    discover_scale_agents,
)
from corvex.utils.available_software import AvailableSoftware
...
def build_model(api_key: Optional[str], model_name: str,
    ...
    the Corvex agents (see vasp_agent.py).

      - If ``base_url`` is set: PNNL-style internal proxy, use the
        OpenAI-compatible wrapper with CORVEX_API_KEY for auth.
      - Otherwise: public LiteLLM path. Resolve the key as
        ...
        the model name) → CORVEX_API_KEY fallback. The last fallback
        is what makes ``CORVEX_API_KEY=<your-anthropic-key>`` work
    ...
    if base_url:
        from corvex.wrappers.openai_wrapper import OpenAIAsGenerativeModel
        if api_key is None:
            api_key = get_internal_proxy_key()
        if not api_key:
            raise APIKeyNotFoundError("internal proxy (CORVEX_API_KEY)")
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "CORVEX_API_KEY (internal proxy); without, infers the "
            "provider from --model-name and uses the matching env var "
            "(GOOGLE_API_KEY / ANTHROPIC_API_KEY / OPENAI_API_KEY)."
        ),
    )
    parser.add_argument(
        "--base-url", default=os.environ.get("CORVEX_BASE_URL"),
        help=(
            "Internal proxy base URL (e.g. PNNL's). Triggers the "
            "OpenAI-compatible wrapper that uses CORVEX_API_KEY for "
            "auth. Defaults to $CORVEX_BASE_URL."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "CORVEX_API_KEY=<your anthropic key> via the auth-fallback "
    ...
    try:
        ...
        print(
            ...
            "(or set $CORVEX_BASE_URL) so CORVEX_API_KEY is used "
