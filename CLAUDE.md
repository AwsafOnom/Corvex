...
Every chat-driven orchestrator in Corvex falls into one of three modes —
...
Going forward, Corvex intends to extend its agentic capabilities primarily through skill
...
`corvex/skills/planning/<name>/` are markdown — no per-skill
...
(`~/.corvex/knowledge_bases/<name>/`, managed by `corvex kb` /
`corvex/knowledge/kb_store.py`) with a `manifest.json` recording the
...
manually — bare `corvex` (or `corvex explore`) launches it. It is **not a
fourth mode**; it's an orchestrator-of-orchestrators with a different role
(router + context bridge). It lives in `corvex/agents/meta_agent/`
...
wired (`delegate_to_simulation`). Because `corvex.agents.sim_agents`
...
and the tool returns a clean "install corvex[sim]" error if `ase` is absent.
...
2. `corvex simulate` CLI flesh-out (replace the "Coming Soon!" stub)
3. HPC backend (`corvex/hpc/` — `Connection`, `Scheduler`) — see PR #140;
...
The meta agent (`corvex/agents/meta_agent/`) was built following this same
...
`corvex/agents/sim_agents/_potential.py`): `MLIPAgent` emits it,
...
- **Skill bundles** at `corvex/skills/<domain>/<name>/` — one folder per
  skill containing `<name>.md` plus optional sibling `.py` helpers
  (Anthropic-Skill shape).
- **Cross-skill helpers** at `corvex/skills/_shared/` — modules referenced
  by multiple bundles, plus the `_registry.py` / `_spec.py` discovery
  infrastructure.
- **Non-skill utilities** at `corvex/utils/`. The legacy `corvex/tools/`
  no longer exists.
...
and imports them as `corvex.skills.…` modules). Skills added via the UI
uploader or dropped in the persistent `~/.corvex` store are markdown-only
...
| Shared library across skills | Not a concept (skills are independent units) | `corvex/skills/_shared/` — always-on infrastructure |

Conceptually: Anthropic skills are *independently distributable units the
model picks at conversation time*; Corvex skills are *in-package knowledge
...
`CORVEX_API_KEY` is the **proxy** key. It pairs with `base_url=<proxy-url>`
...
`GOOGLE_API_KEY`, …). Corvex agent constructors must NOT fall back to
`CORVEX_API_KEY` on this path — when no vendor key is available, raise
